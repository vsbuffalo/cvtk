...
A long-standing problem in evolutionary genetics is quantifying the
roles \gc{genetic} drift
and selection have in shaping genome-wide allele frequency
changes. Seletion can both directly and indirectly affect
allele frequency change, with the indirect effect coming from the
action of selection on \gc{correlated loci elsewhere in genome
  e.g. linked selection \parencite{MaynardSmith,Charleseworths,etc}}.
Previous
work on this question has \gc{mostly} focused on teasing apart the impacts of drift and
selection on genome-wide diversity using population samples from a single
contemporary timepoint, \gc{often} by modeling the correlation between regional
recombination rate, \gc{gene density,} and diversity created in the presence of linked selection
\parencite{Begun1992-ey,Elyashiv2016-vt}. \gc{This} approach has
\gc{shown a major }
role \gc{for} linked selection in shaping patterns
of genome-wide diversity across \gc{the gneomes of} a variety of organisms
...
\gc{However, we} lack an understanding of how \gc{genome-wide} linked selection acts over an important
dimension: time.

Presently, there are numerous examples of \gc{rapid phenotypic adaptation}
\parencite{Grant2011-wk,Grant2006-hj,Reznick1997-mh,Franks2007-dr}
and \gc{rapid genomic evolution in asexual populations \parencite{Lenski,Desai}.}
Yet the
polygenic nature of fitness makes detecting the impact selection has on
genome-wide variation over short timescales \gc{in sexual populations} remarkably difficult. This is
because the effect of selection on a polygenic trait (such as fitness) is
distributed across loci proportional to their effect sizes, \gc{which
  can} lead to subtle  %want to be careful here as Kevin T and
                       %Wolfgang would say that
allele frequency shifts that are difficult to distinguish from background
levels of genetic drift and sampling variance. However, increasingly genomic
\gc{experimental evolution} studies with multiple timepoints and, in some cases, multiple replicate
populations are becoming available, are being used to detect selected loci
\parencite{Turner2011-sx,Turner2012-bm} and differentiate modes of
selection %cite silver fish by Nina and others
\parencite{Burke2010-tz,Barghi2019-qy}. \gc{In addition these temporal-genomic
  studies have begun in wild populations \parencite{Bergland,Machado,Othere}
 } In a previous paper, we proposed that
one signal of polygenic linked selection that could be detected from these
temporal genomic studies is the temporal autocovariance in allele frequency
changes \parencite{Buffalo2019-io}. These covariances are directly estimable
from temporal genomic data and are created when the loci that underly heritable
fitness variation perturbed linked neutral alleles; when genetic drift acts
alone in a closed population, these covariances are zero in expectation.
\gc{The covariances provide a useful tool for understanding the
  gneome-wide imapct of linked selection. First, because it is
  mathematically natural
  to decompose the total variance in temporal allele frequency change
  across a set of time intervals into the covariances in allele
  frequency change among time intervals. Second, because biologically
  the covariances reflect the correlation of selection coefficients
  experienced in different time intervals. }
...
Finally, we observed that in the \gc{longest study}  we   %run evolve-and-resequence 
analyzed \parencite{Barghi2019-qy}, some genome-wide temporal covariances
\gc{become} negative at future timepoints (see the first two rows in
Figure \ref{fig:figure-1} A). \gc{This shows that alleles that were on
average going up initially are no going down in frequency, i.e. that
the average direction of selection experienced by alleles has
flipped. This must reflect either a change in the environment or the
genetic background, due to changing epistatic relationships among
alleles or  recombination breaking up selective alleles.}
\gc{Such reversals of selective dynamics could be occuring at other time-points but} the
signal of a change in the direction of selection \gc{at particular
  loci} may be washed out when we calculate our  genome-wide \gc{average} temporal
covariances. To address this limitation, we
calculated \gc{the distribution of the} temporal covariances over
100kb genomic windows \gc{(Figure \ref{fig:figure-3}, pooling
across all replicates see Supplementary Figure \ref{suppfig:barghi-offset-replicate-panels} for individuals replicates).}
\gc{The}
covariance of each tile \gc{will be} noisy, \gc{due to sampling and genetic
  drift, and the neutral distribution of the covariance is
  complicated due to} linkage
...
\gc{Thus} we have developed a permutation-based procedure that constructs a null
distribution by randomly flipping the signs of the    %of the
                                %genome-wide covariances as they would
                                %look if only neutral genetic drift was acting. We do this 
allele frequency changes per-genomic window. \gc{This destroys}  %%  GC bit about 1,000 times seems more like figgure caption or methods
the systematic covariances created by linked selection but creates a sampling
distribution of the covariances spuriously created by neutral genetic
drift while preserve the complex dependencies
...
variances of the covariances are wider than expected under drift
alone \gc{as they include the effect of selection on the allele
  frequency change within a time-interval, just not between
  time-intervals.} We see, Figure \ref{fig:figure-3} A and B, that
...
temporal covariances tend to shift to become more negative (a heavy
left tail).
We quantified the degree to which the left and right
tails \gc{are inflated compared to the null distribution as a function
  of time}, and see excesses in both tails in Figure
\ref{fig:figure-3} C. This finding is also
robust to sign-permuting allele frequency changes on a chromosome-level, the
longest extent that gametic linkage disequilibria can extend {\bf ????} (Supplementary
Figure \ref{suppfig:barghi-tailprobs-seqid}).  \gc{We} see a striking
pattern that \gc{the windowed} covariances not only decay towards zero, but in fact become
negative through time, consistent with \gc{many} regions in the genome having had a
...
f%lipping the sign at the block-level, we 
...
t%ime. We would like to compare the distribution of these tile covariances to a
