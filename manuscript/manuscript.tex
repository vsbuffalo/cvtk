...
% - change supp fig ordering
% - sig impact
% -ref table 1
% fix equation G(t) in Fig 1

\title{Estimating the genome-wide contribution of selection to temporal allele frequency change}

% Significant Statement:

% Evolutionary biologists and ecologists have repeatedly found evidence of traits
% like flowering time changing rapidly adapting 
...
\begin{abstract}
 ...
 replicates. We estimate that at least 17\% to 37\% of allele frequency
 change \vb{in these populations} is driven by selection in these experiments.
...
distributed across loci in proportion to their effect sizes. This can lead to
subtle allele frequency shifts on standing variation that are difficult to
...
detect large effect selected loci \parencite{Turner2011-sx,Turner2012-bm} and
...
polygenic linked selection detectable from genomic studies \vb{with multiple
...
biologically, these covariances reflect the extent to which \vb{neutral} allele
frequency changes in one generation predict changes in another due to a shared
\vb{selection} pressure \vb{and linkage to selected loci}.
...
frequency changes between replicate populations caused by \vb{the} response to
...
\ref{supp:ind-depth-var-corr} and \ref{supp:matrix-correction}). \vb{These}
...
time intervals, as expected when directional selection affects linked variants'
frequency trajectories until ultimately linkage disequilibrium and the additive
genetic variance for fitness associated with neutral alleles decays
...
One concern is that these covariances reflect the localized impact of a few
large-effect loci rather than selection on a polygenic trait. Since our
covariances are essentially averages over loci, the covariance estimate can be
strongly affected by a single outlier region. To test whether large outlier
...
the impact of selection \vb{acting across} many genomic windows. 
...
(Figure \ref{fig:figure-2} B, 95\% CIs, [0.00635, 0.00860], [0.00636, 0.00859],
[0.00608, 0.00834]). We estimate the convergent correlation coefficient across
these replicates as 0.36 ($95\%$ block-bootstrap confidence interval $[0.31,
0.40]$). Similarly, we can calculate the proportion of the total variance in
allele frequency change from convergent selection pressure analogous to $G$
where the numerator is is the convergent covariance and the denominator is the
total variance (see Supplementary Material \ref{supp:replicate-g}). We find
that 37\% of the total variance is due to shared allele frequency changes
caused by selection (95\% CI [29\%, 41\%]; these are similar to the convergence
correlation, see Supplementary Material \ref{supp:replicate-g}).
...
control line and \vb{each of} the two Longshanks lines. We find that this is
...
in frequency change of the Longshanks selection lines. We can partition the
allele frequency change between the two timepoints (20 generations apart) for a
Longshanks line as \vb{$\Delta p_{t,\mathrm{LS1}} = \Delta_{_{D}}
  p_{t,\mathrm{LS1}} + \Delta_{_{U}} p_{t,\mathrm{LS1}} + \Delta_{_S}
  p_{t,\mathrm{LS}}$} where these terms are the decomposition in the allele
  frequency change due to drift in \vb{Longshanks replicate 1 ($\Delta_{_D}
    p_{t,\mathrm{LS1}}$), selection unique to the LS1 replicate ($\Delta_{_U}
    p_{t,\mathrm{LS1}}$), and selection response shared between the two
    Longshanks replicates ($\Delta_{_S} p_{t,\mathrm{LS}}$) respectively (and
  similarly for the Longshanks two line, LS2)}. By construction we will assume
  that each of these terms are uncorrelated within replicates, and that only
  the shared term covaries between the replicates.  Assuming that we can
  approximate the contribution of genetic drift in the LS lines as the variance
  in allele frequency change observed in the control, i.e.  \vb{$\var(\Delta
  ...
  selected lines as $\cov(\Delta p_{t,\mathrm{LS1}}, \Delta
  p_{t,\mathrm{LS2}})$ (see Supplementary Material Section
  ...
  effects of selection, of which 14\% (95\% CI $[3\%, 33\%]$) is estimated to
  be unique to a selection line, and 17\% (95\% CI $[9\%, 23\%]$) is the effect
  of shared selection between the two Longshanks selection lines.
...
distribution of the temporal covariances over \vb{100kb windowed covariances}
...
contemporary populations. \gc{Thus,} we can hope to have a much richer picture
of the\gc{ impact of selection} as temporal sequencing becomes more common,
\gc{allowing} us to observe the effects of ecological dynamics in genomic data
