...
%DIF DEL old.tex          Tue May 12 17:26:19 2020
%DIF ADD manuscript.tex   Tue May 12 20:56:23 2020
...
frequencies as some parents }\vb{contribute} \DIFadd{more or less copies of specific
...
elsewhere in genome\DIFaddbegin \DIFadd{, }\DIFaddend e.g. linked selection (\cite{Maynard_Smith1974-lc},
...
replicate populations, are being used to detect \DIFdelbegin \DIFdel{large effect }\DIFdelend \DIFaddbegin \vb{large-effect} \DIFaddend selected loci
...
timepoints }\DIFdelend \DIFaddbegin \vb{temporal genomic data} \DIFaddend is the
temporal autocovariance \DIFaddbegin \DIFadd{(i.e. covariance between two timepoints) }\DIFaddend in allele
frequency changes \parencite{Buffalo2019-io}.  These covariances \DIFdelbegin \DIFdel{are directly estimable from
temporal genomic data and are }\DIFdelend \DIFaddbegin \vbout{can be
estimated from temporal genomic data and} \DIFadd{are }\DIFaddend created when the loci that
underly heritable fitness variation perturb the frequencies of linked \DIFdelbegin \DIFdel{neutral }\DIFdelend alleles;
in contrast, when genetic drift acts alone in a closed population, these
covariances are zero in expectation \DIFaddbegin \DIFadd{for neutral alleles}\DIFaddend .  Mathematically,
temporal covariances are useful because it is natural to decompose the total
variance in allele frequency change across a \DIFdelbegin \DIFdel{set of time intervals }\DIFdelend \DIFaddbegin \vb{time interval} \DIFaddend into the
variances and covariances in allele frequency change \DIFdelbegin \DIFdel{among time intervals}\DIFdelend \DIFaddbegin \vb{between generations}\DIFaddend .
Furthermore, biologically, these covariances reflect the extent to which \DIFdelbegin \DIFdel{neutral }\DIFdelend allele
...
change (in ten-generation intervals, denoted $\Delta_{_{10}} p_t$) \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \vb{of} \DIFaddend some
...
biases created due to sampling noise, and \DIFdelbegin \DIFdel{normalize }\DIFdelend \DIFaddbegin \vb{normalized} \DIFaddend the entries for
...
the impact of selection acting across many genomic \DIFdelbegin \DIFdel{windows}\DIFdelend \DIFaddbegin \vb{regions}\DIFaddend . 
...
correct }\DIFdelend \DIFaddbegin \vb{$\var(p_t - p_0) = \sum_{i=0}^{t-1}
  \var(\Delta p_i) + \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i,
\Delta p_j)$} \DIFadd{(we }\vb{bias-correct} \DIFaddend these for sequencing depth), and the
...
\begin{align}
  \DIFdelbegin \DIFdel{G(t) = \frac{\sum_{i\ne j}^{t-1} \cov(\Delta p_i, \Delta p_j)}{\var(p_t - p_0)}
}\DIFdelend \DIFaddbegin \vb{G(t) = \frac{ \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i, \Delta p_j)}{\var(p_t - p_0)}}
\DIFaddend \end{align}
%
\DIFaddbegin 

%DIF > \textcite{Barghi2019-qy}'s design means that we cannot \vb{observe covariances
%DIF > between pairs of generations that constitute the estimated covariances between
%DIF > ten-generation time intervals
%DIF > 
\DIFaddend which estimates the \DIFdelbegin \DIFdel{effect of selection }\DIFdelend \DIFaddbegin \vb{impact of selection} \DIFaddend on allele frequency change between
...
sequenced every ten generations, \DIFdelbegin \DIFdel{in the numerator for the
covariance we use the allele frequency changes between adjacent timepoints, which are ten generations apart. Consequently, this leads }\DIFdelend \DIFaddbegin \vb{the numerator uses the covariances
  estimated between ten-generation blocks of allele frequency change}\DIFadd{; }\vb{thus
  the strong, unobservable, covariances between adjacent generations do not
contribute to the numerator of $G(t)$}\DIFadd{. Had these covariances been measurable
...
the numerator of $G(t)$}\DIFdelend \DIFaddbegin \DIFadd{is }\vb{considerably} \DIFadd{conservative (}\vb{we demonstrate this through
simulations in Supplementary Material Section \ref{supp:tempblocks}}\DIFadd{)}\DIFaddend .  Still,
...
%DIF > \vb{This implies that the $N_e$
%DIF > without the effects of selection is at XXX $\nicefrac{}{(1-G(t))}$}
\DIFaddbegin 
...
\textcite{Barghi2019-qy} data, we use $\Delta_{_{10}} p_t$. \DIFaddbegin \vb{Between
replicate populations with identical allele frequency changes, this is one.}
\DIFaddend 
...
positive \DIFdelbegin \DIFdel{between-replicate covariances.  We find all three pairwise
between-replicate covariances are positive and statistically significant
(Figure \ref{fig:figure-2} B.
We estimate the convergent correlation
coefficient across these replicates as 0.36 ($95\%$ CI $[0.31, 0.40]$).
Similarly, }\DIFdelend \DIFaddbegin \vb{convergence correlations}\DIFadd{.  }\vb{We find all three convergent
correlation coefficients between replicate pairs are significant (Figure
\ref{fig:figure-2}B), and average to 0.36 ($95\%$ CI $[0.31, 0.40]$)}\DIFadd{.
}\vb{Additionally,} \DIFaddend we can calculate the proportion of the total variance in
allele frequency change from convergent selection pressure\DIFdelbegin \DIFdel{analogous to $G$ }\DIFdelend \DIFaddbegin \DIFadd{, }\vb{analogous to
...
experiment offers a useful control to test our \DIFdelbegin \DIFdel{between-replicate covariances}\DIFdelend \DIFaddbegin \vb{convergence correlations}\DIFaddend :
we expect to see \DIFdelbegin \DIFdel{positive
between-replicate covariance }\DIFdelend \DIFaddbegin \vb{significant} \DIFadd{positive }\vb{convergence correlations} \DIFaddend in the
comparison between the two Longshanks selection lines, but not \DIFdelbegin \DIFdel{between the two pairwise comparisons between the
control line and each of the two Longshanks lines}\DIFdelend \DIFaddbegin \vb{between each
of the control line and Longshanks line pairs}\DIFaddend . We find that this is the case
(gray confidence intervals in Figure \ref{fig:figure-2}C), with \DIFdelbegin \DIFdel{the
two Longshanks comparisons to the control line not being significantly
different from zero}\DIFdelend \DIFaddbegin \vb{convergence
correlations between each of the Longshanks lines to the control not being
statistically different from zero}\DIFaddend , while the \DIFdelbegin \DIFdel{comparison }\DIFdelend \DIFaddbegin \DIFadd{convergence correlation }\DIFaddend between
the two Longshanks lines is \DIFdelbegin \DIFdel{statistically significantly different from zero }\DIFdelend \DIFaddbegin \vb{strong (0.18)} \DIFadd{and statistically significant
}\DIFaddend (CIs [\DIFdelbegin \DIFdel{0.0129, 0.0400}\DIFdelend \DIFaddbegin \DIFadd{0.07, 0.25}\DIFaddend ]).
...
This \DIFdelbegin \DIFdel{must }\DIFdelend \DIFaddbegin \vb{might} \DIFaddend reflect either a change in the environment or the genetic
background, due to epistatic relationships among alleles altered by frequency
changes \DIFaddbegin \DIFadd{(}\vb{which can occur during an optima shift; \cite{Hayward2019-kt}}\DIFadd{) }\DIFaddend or
recombination breaking up selective alleles. Such reversals \DIFdelbegin \DIFdel{of selective dynamics }\DIFdelend \DIFaddbegin \DIFadd{in }\vb{selection
dynamics} \DIFaddend could be occurring at other timepoints but the signal of a change in
...
an empirical \DIFaddbegin \vb{neutral} \DIFaddend null distribution by randomly flipping the \DIFdelbegin \DIFdel{signs }\DIFdelend \DIFaddbegin \DIFadd{sign }\DIFaddend of
...
include the effect of selection on the }\DIFdelend \DIFaddbegin \DIFadd{, as selection }\vb{not only creates covariance between time
...
}\vb{$G(t)$}\DIFadd{) over the relatively short time periods similar to our analyzed
selection datasets span (Figure \ref{fig:figure-4}C).  The magnitude of
}\vb{$G(t)$} \DIFadd{increases with the strength of selection, i.e. the variance in
...
underlying the trait, suggesting that they }\vb{are} \DIFadd{capturing highly polygenic
signals.  Indeed if only a small number of loci influence the trait, the $G(t)$
trajectories are typically much more stochastic }\vb{across replicates} \DIFadd{(compare
...
\DIFdelbegin \DIFdel{such a
}\DIFdelend \DIFaddbegin \vb{even a} \DIFaddend strong effect of linked selection may not be differentiable from
genetic drift using only single contemporary population samples or looking 
at \DIFdelbegin \DIFdel{at }\DIFdelend temporal allele frequency change at each locus in isolation. In this way,
...
these populations is due to the (\DIFaddbegin \vb{likely} \DIFaddend indirect) action of selection.
...
across replicate populations, and found that a \DIFdelbegin \DIFdel{reasonable }\DIFdelend \DIFaddbegin \vb{substantial} \DIFaddend proportion of
...
patterns of \DIFdelbegin \DIFdel{LD }\DIFdelend \DIFaddbegin \vb{linkage disequilibria (LD)} \DIFaddend and recombination which would allow
...
timescales that linked selection \DIFdelbegin \DIFdel{act over }\DIFdelend \DIFaddbegin \vb{acts} \DIFadd{over }\vb{in the wild}\DIFaddend . In our
...
\DIFdelbegin \DIFdel{only from }\DIFdelend \DIFaddbegin \DIFadd{from }\vb{only} \DIFaddend contemporary populations. Thus, we can hope to have a much
...
\textcite{Kelly2019-dc}, \DIFdelbegin \DIFdel{\textcite{Bergland2014-ij}, and
\textcite{Castro2019-uk}}\DIFdelend \DIFaddbegin \DIFadd{and \textcite{Bergland2014-ij}, }\vb{and
individual-level sequencing data from \textcite{Castro2019-uk}}\DIFaddend . In all cases,
...
}\DIFdelend \DIFaddbegin \vb{an allele} \DIFadd{to track
}\vb{frequency} \DIFaddend for each locus. Then, we calculate the variance-covariance
...
resamples blocks of \DIFdelbegin \DIFdel{data points}\DIFdelend \DIFaddbegin \vb{loci}\DIFaddend , rather than individual \DIFdelbegin \DIFdel{data points}\DIFdelend \DIFaddbegin \vb{loci}\DIFaddend , to infer the
...
%DIFDELCMD < %%%
\DIFdel{We would like to thank the authors of }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Forward-in-time Simulations}}

\DIFadd{To explore how aspects of genetic architecture, model of selection, and
experimental design impact temporal covariance, }\DIFaddend the \DIFdelbegin \DIFdel{original studies we 've analyzed, including Neda Barghi, Christian Schl}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{tterer, John Kelly, Kimberly Hughes, Frank Chan, Campbell Rolian, Nick Barton, Alan Bergland, and Dmitri Petrov.  We would also like to thank Doc Edge for helpful statistical advice, and Matt
Osmond, Erin Calfee, and
Sivan Yair for helpful discussions. This research was supported by an NSF Graduate Research Fellowship grant awarded to VB (1650042), and NIH (R01-GM108779)and NSF (1353380)awarded to GC. }\DIFdelend \DIFaddbegin \DIFadd{$G(t)$ trajectories, and
...
\DIFaddbegin \DIFadd{We simulated directional selection on a trait by first burning in a population
...
(these population sizes }\vb{varied between} \DIFadd{50, 500, and 1000 diploids; the
...
pattern does not follow that of seasonal fluctuating selection\DIFaddbegin \DIFadd{.  
}


\subsection{\DIFadd{Approximating the Reduction in Diversity from $G(t)$}}
\label{supp:reduction-approx}

\DIFadd{To help reconcile our measure of linked selection, $G(t)$, with familiar
}\vb{expressions as a reduction} \DIFadd{in neutral diversity, as parameterized by
...
%DIF > 
\DIFadd{where $\varepsilon \ge 0$ since these variances are positive. }\vb{Throughout
...
}\vb{replicate} \DIFadd{population simulations to investigate how convergence
...
are multiple routes to adaptation}\DIFadd{. We should note that our simplified
...
probability, trait mutation rate $10^{-8}$}\DIFadd{, and the optima shift began at five
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{Mean temporal covariance ($\cov \Delta p_5, p_t$, with $t$ varying
...
which is close to the bottom end of our parameter range.} \DIFadd{We also vary the
...
\DIFadd{We }\vb{also} \DIFadd{explored how directional truncation selection generates temporal
covariances. In these simulations we select the top }\vb{10\%, 25\%, or 50\%} \DIFadd{of
...
alleles are selectively neutral until directional selection is imposed}\DIFadd{. More
...
at later timepoints} \DIFadd{was not spuriously caused by the way fixed or lost sites
...
when sites with frequency zero or one are excluded}\DIFaddend .
