...
during burnin, with two optima shift routines: (1) sudden optima shifts of
$\mu_\text{sudden} = \{0.1\sigma, 0.5\sigma, 1\sigma\}$, and (2) very graduate
optima shifts of $\mu_\text{gradual} = \{0.001\sigma, 0.01\sigma\}$ per
generation using the same two population simulation scheme described above. We
used a polygenic architecture for these simulations ($L = 1000$ trait affecting
loci), and the optima shift began at five generations after a $10N$ generation.
Across our GSS simulations, we see the expected selection response
(Supplementary Material Figure \ref{suppfig:gss-zbar}).
...
$G'(t)$, the latter of which includes these variance terms
...
pairwise diversity in a region under background selection is invariant with
...
the $r_{BP} = 10^{-8}$ subfigure (Supplementary Material Figure
\ref{suppfig:supp-bgs-g}). However, further future work is needed to fully
...
carrying deleterious alleles between the two populations.

Note that to investigate whether different selection coefficients converged to
...
the sum of pairwise temporal covariances could be slightly negative. 

When all pairwise covariances are calculated and
summed to calculate the numerator of $G(t)$, this leads to this subset of
neutral trajectories to have spuriously significant and positive $G(t)$
trajectories. We briefly confirmed that conditioning on fixation leads to a
spurious positive numerator of $G(t)$. Looking at one neutral simulation, we
conditioned on alleles fixing during the 105 generations of which temporal
covariances are calculated. Per locus, we calculated the sum of all pairwise
products between allele frequency changes (analogous to the numerator of
$G(t)$). Whether we condition on fixation, or a large absolute allele frequency
change between the initial generation and the last generation, we find that
quickly fixing sites and sites with large allele frequency shifts have a
significant positive sum of covariances as expected, despite being neutral
trajectories. Thus, even under neutrality, one must mind how fixations are
handled. 



Since our simulation allele
frequencies are population, rather than sample frequencies, when an allele is
fixed or lost is known with certainty. 
