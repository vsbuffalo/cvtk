...
with theory; \gc{the temporal covariance in allele frequencies decay}
as \gc{presumably due to the} linkage disequilibria and additive variation \gc{for
fitness present in a particular generation} decay through time \graham{So we need to be careful here. As
  the additive var for fitness may be constant. It's that the VA from
  a particular gen is decaying. Not sure if }. In reanalyzing one study
...
the \cite{Barghi2019-qy} study) to 0.36 (in the \cite{Kelly2019-dc}
study). \graham{Are these just repeat numbers from main text? If so
  nix this sentence}
...
generations \gc{ \parencite{Buffalo}}. This leads to a clear prediction: regions of higher linkage
...
covariance and LD. \graham{Is important point. We'll likely need plots for supp}
...
%more complex than expected under a model of
%directional selection occurring until fitness variation is exhausted

Finally, in re-analyzing the \textcite{Barghi2019-qy} study, we find evidence
of \gc{complex} linked selection dynamics. This is
...
selection over short timescales \gc{will help to} unite phenotypic studies of
