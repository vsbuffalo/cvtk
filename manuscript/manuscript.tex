...
\begin{center}
  ...
  \hline
    ...
    \textcite{Barghi2019-qy} & \emph{D. simulans} & lab & 10 & $\sim$1000 & 60 & \gc{6?} \\ \hline
...
seven timepoints and ten replicate populations, we estimated 
genome-wide $6 \times 6$ temporal covariance matrix $\mathbf{Q}$ for each of
the ten replicates. Each row of these matrices represent the temporal
covariance $\cov(\Delta_{_{10}} p_s, \Delta_{_{10}} p_t)$, between
\gc{the allele frequency change ($\Delta_{_{10}} p_t$) in} some initial
reference generation $s$ (the row of the matrix), and some later timepoint $t$
(the column of the matrix). \gc{We corrected these matrices for biases
created due to sampling noise, and normalize the entries for heterozygosity} (see Supplementary Material Sections
\ref{supp:ind-depth-var-corr} and \ref{supp:matrix-correction} for details on
the bias correction).\gc{ The %off-diagonal
 covariances} are expected to be zero when only drift is acting in
 \gc{a closed} population, as only linked selection can create covariance between allele
 frequency changes \parencite{Buffalo2019-io}. \gc{DO WE WANT to say
   heritable rather than linked sel} \gc{Averaging across the ten
   replicate temporal covariances matrices, Figure \ref{fig:figure-1},} we find \gc{many}
temporal covariances that are statistically significant (95\% block
bootstraps \gc{CIs} do not contain zero),
consistent  % first off-diagonal elements
with linked selection perturbing \gc{genome-wide} allele frequency changes over very short time
periods. \gc{The }covariances between \gc{all adjacent time intervals are}
positive, and then decay towards zero \gc{as we look at more distant
  time intervals} as expected when directional selection
affects linked variants' frequency trajectories until ultimately linkage
disequilibrium and additive genetic variance \gc{for fitness} decay
\parencite{Buffalo2019-io}. \gc{The temporal covariances per replicate
  are noiser but this general pattern holds}; see Supplementary
Figures XXX. \gc{ \textcite{Barghi2019-qy}'s design means that the covariances we see in adjacent time intervals
 are on average ten generations apart, given the temporal decay in
 covariance we see, the covariances on shorter
 time-scales may well be higher yet (see Supplementary Material
 Section \ref{supp:barghi-covs} for more details).}
...
populations experiencing convergent selection pressure\gc{s}. These between-replicate
...
covariance matrices\gc{.} Like temporal covariances, we visualize
these  %which unlike temporal covariance matrices have diagonal
...
(upper left in Figure \ref{fig:figure-2} A). We find these
\gc{covergent covariances} decay very quickly,
from an initial \gc{value} of about 0.1 (95\% block
...
of \textcite{Kelly2019-dc}, which evolved three replicate \gc{wild} populations of
\emph{Drosophila simulans} for 14 generations \gc{adapting to} a novel laboratory
...
block-bootstrap confidence interval $[0.31, 0.40]$). \gc{GIVE
  conversion \% var due to linked selection. }
...
analyzed \parencite{Barghi2019-qy}, some \gc{genome-wide} temporal covariances appeared to
...
\parencite{Benjamini1995-jy},however, their bionomial null model does not
