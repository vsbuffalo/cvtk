...
We first analyzed the dataset of Barghi et al. (2019), an
...
as a population reaches a new optimum, and directional selection weakens)
...
covariance, establishing that genomic temporal covariances are non-zero
...
\end{align}
%
This estimates the impact of selection on allele frequency change between
...
is positive, this measure can intuitively be understood as a lower bound on
...
details).  Additionally, selection inflates the variance in allele
...
\begin{figure*}
  ...
  \caption{Forward-in-time simulations demonstrate how temporal covariance,
    ...
    (see SI Appendix Section \ref{supp:tempcov-gss} for details on these
    simulations).}
...
\ref{suppfig:sim-expfit-Gs}). Furthermore, using our simulations we find
...
SI Appendix, Fig.  \ref{suppfig:supp-blocks}).

Additionally, we explored other modes of selection with simulations.
...
some of this signature may be ongoing purifying selection. Only in the
case of the Longshanks experiment, does the control line allow us to conclude
that selection that is almost entirely due to the novel selection
pressure.
...
convergence correlations, we ran extensive forward-in-time simulations
...
\ref{supp:forward} describes these simulation routines and others in detail.
