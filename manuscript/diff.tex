...
%DIF DEL old.tex          Sun May 10 09:17:32 2020
%DIF ADD manuscript.tex   Sat May  9 21:17:04 2020
...
\begin{abstract}
 ...
 change \DIFdelbegin \DIFdel{in neutral allele }\DIFdelend \DIFaddbegin \gc{in allele} \DIFaddend frequency at one timepoint is predictive of the
 changes at later timepoints when there is heritable fitness variation in the
 population, \DIFdelbegin \DIFdel{as neutral alleles }\DIFdelend \DIFaddbegin \gc{as alleles} \DIFaddend can remain associated with selected alleles
 over time \DIFaddbegin \gc{and so be indirectly selected for}\DIFaddend . Since genetic drift does not lead to temporal covariance, we can
 ...
 reanalyze \DIFdelbegin \DIFdel{two }\emph{\DIFdel{Drosophila simulans}} %DIFAUXCMD
\DIFdel{evolve-and-resequence studies, and
 one artificial selection experiment in mice}\DIFdelend \DIFaddbegin \vb{three selection experiments}\DIFadd{, }\vbout{two \emph{Drosophila
   simulans} evolve-and-resequence studies, and one artificial selection
 experiment in mice}\DIFaddend , to quantify the effects of linked selection over short
...
\DIFaddend with the indirect effect coming from the action of selection on \DIFdelbegin \DIFdel{correlated }\DIFdelend \DIFaddbegin \vb{linked}
\DIFaddend loci elsewhere in genome e.g. linked selection (\cite{Maynard_Smith1974-lc},
...
of linked \DIFdelbegin \DIFdel{neutral alleles}\DIFdelend \DIFaddbegin \gc{alleles}\DIFaddend ; in contrast, when genetic drift acts alone in a
closed population, these covariances are zero in expectation \DIFaddbegin \gc{for
  neutral alleles}\DIFaddend .  Mathematically,
...
\DIFdelbegin \DIFdel{neutral allele
}\DIFdelend \DIFaddbegin \gc{allele} \DIFaddend frequency changes in one generation predict changes in another
...
linkage disequilibrium and the \DIFaddbegin \gc{associated} \DIFaddend additive genetic
variance for fitness \DIFdelbegin \DIFdel{associated
with neutral alleles decays }\DIFdelend \DIFaddbegin \DIFadd{decays (}\vb{which could occur as a population reaches
a new optimum}\DIFadd{) }\DIFaddend \parencite{Buffalo2019-io}. The temporal covariances per
...
to see how this quantity grows through time. \DIFaddbegin \vb{This measure can intuitively
...
looks like a faster rate of drift, or lower $N_e$.} \DIFaddend Since
...
adjacent timepoints, which are ten generations apart\DIFaddbegin \DIFadd{; }\vb{thus the strong
  covariances between adjacent generations are not observable and do not
  contribute to the numerator of $G(t)$}\DIFadd{. }\vb{Additionally, selection also
...
reasons, our measure $G(t)$ is strongly conservative} \DIFadd{(}\vb{we demonstrate this
through simulations in Supplementary Material Section \ref{supp:tempblocks}}\DIFadd{)}\DIFaddend .
\DIFdelbegin \DIFdel{Consequently, this leads our measure $G(t)$ to
...
the numerator of $G(t)$. }\DIFdelend Still, we find a remarkably strong signal. Greater than $20\%$ of total,
...
covariances: \DIFdelbegin \DIFdel{neutral alleles }\DIFdelend \DIFaddbegin \gc{alleles} \DIFaddend linked to a particular fitness background are
...
reflect that \DIFdelbegin \DIFdel{neutral alleles }\DIFdelend \DIFaddbegin \gc{alleles} \DIFaddend associated with heritable fitness backgrounds are
...
\DIFdelbegin \DIFdel{were maintained at very small effective population sizes, estimated
at 300, 450, and 45 for the \textcite{Barghi2019-qy}, \textcite{Kelly2019-dc},
and \textcite{Castro2019-uk} studies respectively, }\DIFdelend \DIFaddbegin \gc{were maintained at relatively small census sizes (Table
\ref{table:study_summary})}\DIFadd{, }\DIFaddend which will amplify the role of genetic drift. The
advantage of lab experiments is that they are closed populations\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \vb{;} \DIFaddend in
natural populations temporal covariance could also arise from the systematic
migration of alleles from differentiated populations. Adapting these methods to
natural populations will require either populations that are reasonably closed
to migration, or for the effect of migration to be accounted for possibly
either by knowledge of allele frequencies in source populations or the
...
(Supplementary Figure \ref{suppfig:bergland-correction}\DIFdelbegin \DIFdel{, XXX).  }\DIFdelend \DIFaddbegin \DIFadd{).  }\vb{Through our
...
\DIFadd{To help help reconcile our measure of linked selection, $G(t)$, with familiar
reductions  }\gc{in neutral diversity, as parameterized by} \DIFadd{$N_e$, we develop some rough
approximations here. First, we introduce some convenient notation. Let $V_{T} =
...
linked selection has on the variances in allele frequency change. 
}
...
\DIFadd{where $\varepsilon \ge 0$ since these variances must be positive.
}

\gc{In a neutral} \DIFadd{Wright--Fisher population, 
}
...
\nicefrac{\bar{\pi}}{\pi_{\gc{0}}}$. 
}

\gc{\cite{Elyashiv2016-vt} estimated that linked selection in \emph{Drosophila
  melanogaster} had resulted in a $1 -
\nicefrac{\bar{\pi}}{\pi_{\gc{0}}} = 77\%$ reduction in the level of
neutral diversity. Thus our estimate of $G(t) \approx 20\%$ in \emph{Drosophila
  simulans} is smaller
than that seen over long timespans in a closely related
species. }
...
\subsection{\DIFadd{Forward-in-time Simulations}}
\label{supp:forward}
\vb{
...
Osmond, Erin Calfee, \DIFdelbegin \DIFdel{and Sivan Yair}\DIFdelend \DIFaddbegin \DIFadd{Andy Kern, Sivan Yair, }\vb{Chuck Langley, Dave Begun, and
Michael Turelli} \DIFaddend for helpful discussions. \DIFaddbegin \vb{Additionally, we thank Guy Sella
and one additional anonymous reviewer for greatly improving the manuscript}\DIFadd{.
}\DIFaddend This research was supported by an NSF Graduate Research Fellowship grant
