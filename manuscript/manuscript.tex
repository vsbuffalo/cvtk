...
\title{Estimating the \gc{genome-wide} contribution of selection to temporal allele frequency change.}
...
% very short timescales.
 \gc{Rapid phenotypic adaptation is often observed in natural populations and selection experiments.}
 However, detecting the \gc{impact of this selection} genome-wide is
  difficult, as\gc{such} adaptation often proceeds from standing
  variation \gc{and} selection on \gc{highly} polygenic traits, both of which may leave faint
  ...
  time-points and across replicates. \gc{We estimate that over 20\% {\bf ??} of allele frequency
  change in these experiments is driven by selection.} \gc{Against
  this background of positive genome-wide tmeporal covariances} We also
  identify signals of negative temporal covariance corresponding to reversals
  in the direction of selection for a reasonable proportion of loci over the time
  course of a selection experiment.  Overall, we find that in the three studies
  we analyzed, linked selection has a \gc{large} impact on short-term allele
  frequency dynamics \gc{that is readily distinguishable from} genetic drift.

  %  Additionally, taking advantage of
  %selection on replicated populations, we can estimate the covariance in allele
  %frequency between replicates due to convergent selection
  %\gc{pressures}.
...
change to the total variance in allele frequency change. \vb{We denote the
...
\end{align}
%
\vb{which estimates} the effect of linked selection between the initial
...
in the numerator of $G(t)$. Still, we find a \vb{remarkably strong signal.
Greater} than $20\%$ of total variation in allele frequency change over 60
generations is the result of linked selection.

\vb{Additionally, we looked for a signal of temporal autocovariance in
...
in more depth in Supplementary Materials Section \ref{bergland-reanalysis}.}
...
among alleles \vb{altered by frequency changes} or recombination breaking up
...
difficult endeavor \parencite{Przeworski2005-bg} \vb{addressed using} new coalescent
models \parencite{Berg2015-xj} and machine learning methods
\parencite{Schrider2017-yx}. An \vb{alternative} approach to understanding the
...
two large effect loci, \vb{is a highly polygenic trait).}
...
One limitation \vb{of these analyses is} that none of the studies we reanalyzed
...
selection. However, with only three cases \vb{analyzed}, further studies on
