...
\begin{abstract}
 ...
 replicates. We estimate that over \vb{17\% to 37\%} of allele
...
\parencite{Begun2007-bg,Beissinger2016-cm,Sattath2011-dr,Williamson2014-oy,Andersen2012-bj,Cutter2010-gi,Elyashiv2016-vt},
...
\cite{Nordborg2005-dc,McVicker2009-ax,ANDOLFATO,MACPHERSON,Hernandez2011-gs,Elyashiv2016-vt}.
...
is acting, as only heritable variation for fitness can create covariance
between allele frequency changes in a closed population
\parencite{Buffalo2019-io}.  Averaging across the ten replicate temporal
...
as expected when directional selection affects linked variants' frequency
trajectories until ultimately linkage disequilibrium and the additive genetic
variance for fitness associated with neutral alleles decays
\parencite{Buffalo2019-io}. The temporal covariances per replicate are noisier
but this general pattern holds; see Supplementary Figures
...
\vb{One concern is that these covariances reflect the localized impact of a few
  ...
  along the genome (we refer to these as windowed covariances
  throughout) and take the median windowed covariance, and
  trimmed-mean windowed covariance, as a measure of the genome-wide
  covariance robust to large-effect loci. These robust estimates (Supplementary Table \ref{supp:table-trimmed-mean})
  confirm the patterns we see using the mean covariance, confirming that genomic temporal covariances are
 non-zero due to the impact of selection on many genomic windows. 

%  This median covariance is also limiting result of a trimmed
 % mean that excludes the upper and lower $\alpha$ tails to calculate the
 % trimmed average windowed covariance. As $\alpha$ increases to 0.5, the
 % trimmed covariance converges to the median windowed covariance (by the
 % definition of the median; see Supplementary Figure
 % \ref{suppfig:barghi-trimmed-mean}). Thus our genomic temporal covariances are
 % non-zero due to the impact of selection on many genomic windows.  }

%We
%  take the median windowed covariance as a measure of the genome-wide
 % covariance robust to large-effect loci; we find that the median windowed
 % covariance $\cov(\Delta p_0, \Delta p_{10})$ (pooling all replicates) is also
 % significantly different from zero, 0.0016 (0.95\% CIS [0.0015, 0.0017], see
 % Supplementary Table \ref{supp:table-trimmed-mean} for more timepoints and
 % trimmed means). 
...
into variance and covariance components, $\var(p_t - p_0) = \sum_{i}
\var(\Delta p_i) + \sum_{i \ne j} \cov(\Delta p_i, \Delta p_j)$, and the
...
study design of \textcite{Barghi2019-qy} leads our measure $G(t)$ to be even
more conservative, since the temporal covariances within each ten-generation
block between sequenced timepoints are not directly observable, and are not
included in the numerator of $G(t)$. Still, we find a remarkably strong signal.
Greater than $20\%$ of total, genome-wide allele frequency change over 60
...
(95\% CIs [0.0087, 0.015]) within 20 generations. This \gc{suggests
  that while a reasonable fraction of the initial response is shared over the
  replicates while the rapid decay} is consistent with
the primary finding of the original \textcite{Barghi2019-qy} study: that
alternative loci contribute to longer term adaptation in \gc{the} different
replicates. 
...
\begin{figure}[!htb]
  ...
  \caption{{\bf A}: The convergence correlation, averaged across \gc{\textcite{Barghi2019-qy}} replicate
    ...
    depicted to the right.  \gc{We note that convergent correlation
      for the last timepoint is an outlier; we are unsure as to the cause of this, e.g. it does not appear to be driven by a single pair
of replicates.} {\bf B}: The convergence covariance between
...
block-bootstrap confidence interval $[0.31, 0.40]$). Similarly, we can
  ...
  XXX). We find that 37\% of the total variance is due to shared allele
frequency changes caused by selection (95\% CI [29\%, 41\%]; these are similar
to the convergence correlation \gc{as the variances are relatively
  constant across replicates, see {\bf XXX}).}

\gc{Next,} we reanalyzed the Longshanks selection experiment, which selected for
...
\gc{One} finding in the Longshanks study were two major-effect loci that showed
...
number of loci. This is consistent with a signal of selection on a
...
\gc{The presence of an unselected control line provides an alternative way to
  ...
  total variance in frequency change of the Longshanks selection
  lines.
We can partition the allele frequency change between the two timepoints (20
  generations) for a Longshanks line as $\Delta p_{t,\mathrm{LS1}} =
  \Delta_{_D} p_{t,\mathrm{LS1}} + \Delta_{_{U1}} p_{t} +
  \Delta_{_\mathrm{S}} p_t$ 
  $\Delta_{_D1} p_{t}$, $\Delta_{_U} p_t$,
  $\Delta_{_\mathrm{LS}} p_t$  the allele frequency change due to
  drift in replicate 1 ($D1$),
  selection unique to the 1$^{st}$ LS replicate ($U1$), and selection response shared between the
  two Longshanks replicates (LS) respectively (and similarly for the Longshanks two line, LS2).
  %partition the allele frequency change between the two timepoints (20
  %generations) for a Longshanks line as $\Delta p_{t,\mathrm{LS1}} =
  % \Delta_{_D} p_{t,\mathrm{LS1}} + \Delta_{_{U}} p_{t,\mathrm{LS1}} +
  % \Delta_{_\mathrm{LS}} p_t$ (and similarly for the Longshanks two line, LS2),
  % and $\Delta p_{t,\mathrm{Ctrl}} = \Delta_{_D} p_{t,\mathrm{Ctrl}} +
  % \Delta_{_{U}} p_{t,\mathrm{Ctrl}} + \Delta_{_\mathrm{Ctrl}} p_t$ for the
  % control line. Here, $\Delta_{_D} p_{t}$, $\Delta_{_U} p_t$,
  % $\Delta_{_\mathrm{LS}} p_t$ are the allele frequency changes due to drift,
  % selection unique to a replicate, and selection response shared between the
  % two Longshanks replicates, respectively.
  By construction we will assume that each of these terms
  are uncorrelated, and we will assume that we can approximate the
  contribution of genetic drift in the LS
  lines the variance in allele frequency change observed in
  the control ($\var(\Delta p_{t,\mathrm{Ctrl}})
  = \var(\Delta_{_D1} p_{t})=\var(\Delta_{_D2} p_{t})$)
  This allows us to estimate the increase in variance in allele
  frequency change due to selection as $(\var(\Delta p_{t,\mathrm{LS1}}) +
  \var(\Delta p_{t,\mathrm{LS2}}))/2 - \var(\Delta p_{t,\mathrm{Ctrl}})$ and
  the shared effect of selection across selected lines as $\cov(\Delta
  p_{t,\mathrm{LS1}}, \Delta p_{t,\mathrm{LS2}})$. We estimate at least 32\%
    (95\% CI $[21\%, 48\%]$) of the variance in allele frequency change is
    driven by the effects of selection, of which 14\% (95\% CI $[3\%, 33\%]$)
    is estimated to be unique to a selection line, and 17\% (95\% CI $[9\%,
  23\%]$) is the effect of shared selection between the two Longshanks
selection lines.}
...
\gc{ Here we report median and trimmed mean of the windowed covariances. We note that
  the median covariance is also limiting result of a trimmed
  mean that symmetrically excludes the upper and lower $\alpha$ tails to calculate the
 trimmed average windowed covariance. As $\alpha$ increases to 0.5, the
  trimmed covariance converges to the median windowed covariance (by the
 definition of the median; see Supplementary Figure
  \ref{suppfig:barghi-trimmed-mean}). Thus our genomic temporal covariances are
 ...
 \begin{table}
   ...
   \centering
   \begin{tabular}{l c c c c c c}
     s & t & 5\% CI & median & 95\% CI & trimmed mean \\ \hline

     0 & 10 & 15.3 & 16.3 & 17.5 & 0.19 \\ \hline
     0 & 20 & 2.8 & 3.7 & 4.6 & 0.05 \\ \hline
     0 & 30 & 4.0 & 4.8 & 5.9 & 0.05 \\ \hline
     0 & 40 & -0.1 & 0.6 & 1.5 & 0.0 \\ \hline
     0 & 50 & -2.7 & -2.0 & -1.3 & -0.03 \\ \hline
     10 & 20 & 14.3 & 15.5 & 16.6 & 0.17 \\ \hline
     10 & 30 & 3.4 & 4.4 & 5.4 & 0.05 \\ \hline
     10 & 40 & 1.5 & 2.3 & 3.3 & 0.03 \\ \hline
     10 & 50 & -4.4 & -3.5 & -2.9 & -0.03 \\ \hline
     20 & 30 & 18.6 & 19.8 & 21.1 & 0.22 \\ \hline
     20 & 40 & 7.0 & 7.9 & 8.9 & 0.09 \\ \hline
     20 & 50 & 0.4 & 1.2 & 2.1 & 0.02 \\ \hline
     30 & 40 & 12.1 & 13.0 & 14.3 & 0.14 \\ \hline
     30 & 50 & -0.4 & 0.7 & 1.8 & 0.01 \\ \hline
     40 & 50 & 12.7 & 13.6 & 14.5 & 0.15 \\ \hline
