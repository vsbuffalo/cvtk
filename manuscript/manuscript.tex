...
under a more complicated, but more realistic fitness model: Gaussian
stabilizing selection. To this end we simulated both sudden optima shifts of
$\mu_\text{sudden} = \{0.1\sigma, 0.5\sigma, 1\sigma\}$ and very graduate
...
show these convergence correlations for the sudden optima shifts in
Supplementary Figure \ref{suppfig:convergence-corrs-gss}. We found that for
slow moving optima shifts, the convergence correlations are generally too weak
to be distinguished from noise reliably, so these results are excluded (but see
Supplementary Materials Section XXX).
