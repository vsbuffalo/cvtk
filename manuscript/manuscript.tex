...
\begin{figure}[!htb]
  ...
  \caption{Forward-in-time simulations of demonstrate how temporal covariance,
    ...
    populations split from a common population, that underwent either an optima
    shift in the same direction (converge) and opposite directions (diverge)
    at generation five. In subfigures (B), (C), and (D), directional selection
    begins at generation five, when the optima shifts; this is indicated by the
...
reliably generate temporal covariances (compare Figure \ref{fig:figure-1}B to
Supplementary Material Figure \gc{XXX}). \gc{This again suggests that
...
selection (Supplementary Material Figure XXX).  We also conducted simulations
of purifying selection alone (i.e. background selection) and find that this can
also generate positive temporal covariances (Figure XXX), thus it is unlikely
that all of the allele frequency change we see is the result of the novel
...
experiment, does the control line mean that we can quantify selection that is
due to the novel selection pressure.
...
%By using an exponential fitness function (corresponding to
%multiplicative selection across sites), \vb{we ensure there is no build up of
%LD beyond what is initially present due to selection and epistasis}
%\parencite{Turelli1990-kd,Bulmer1980-zo}; this serves as the simplest
%directional selection model of a trait to understand the effects of genetic
%architecture on the statistics we have used in the paper.
