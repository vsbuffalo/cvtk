...
Since our covariances are \gc{averages} over loci, the covariance
...
%\textcite{Barghi2019-qy}'s design means that
%we cannot \vb{observe covariances between pairs of generations that constitute
%the estimated covariances between ten-generation time intervals
%
...
sequenced every ten generations, \vb{the numerator uses the covariances
  ...
  contribute to the numerator of $G(t)$}. \gc{Had these
covariances been measurable on shorter timescales, their cumulative
effect would likely have been higher yet (see Supplementary Material Sections
\ref{supp:barghi-covs} and \ref{supp:tempblocks} for more details).}
Additionally, selection also inflates
...
drift alone, as selection \vb{not only creates covariance between time
...
\vb{$G(t)$}) over the relatively short time periods similar to our analyzed
selection datasets span (Figure \ref{fig:figure-4}C).  The magnitude of
\vb{$G(t)$} increases with the strength of selection, i.e. the variance in
...
underlying the trait, suggesting that they \vb{are} capturing highly polygenic
signals.  Indeed if only a small number of loci influence the trait, the $G(t)$
trajectories are typically much more stochastic \vb{across replicates} (compare
...
\vb{even a} strong effect of linked selection may not be differentiable from
...
these populations is due to the (\vb{likely} indirect) action of selection.
...
across replicate populations, and found that a \vb{substantial} proportion of
...
patterns of \vb{linkage disequilibria (LD)} and recombination which would allow
...
timescales that linked selection \vb{acts} over \vb{in the wild}. In our
...
from \vb{only} contemporary populations. Thus, we can hope to have a much
...
\textcite{Kelly2019-dc}, and \textcite{Bergland2014-ij}, \vb{and
...
the reference or minor allele, we randomly choose \vb{an allele} to track
\vb{frequency} for each locus. Then, we calculate the variance-covariance
...
resamples blocks of \vb{loci}, rather than individual \vb{loci}, to infer the
...
(these population sizes \vb{varied between} 50, 500, and 1000 diploids; the
