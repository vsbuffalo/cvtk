...
%DIF DEL old.tex          Mon May 11 09:36:57 2020
%DIF ADD manuscript.tex   Mon May 11 09:35:40 2020
...
\DIFdel{evolve-and-resequence studies, and
 one artificial selection experiment in mice, }\DIFdelend \DIFaddbegin \vb{three selection experiments} \DIFaddend to quantify the
...
processes that shape the genetic composition of populations. In a population
closed to migrants, the two processes that change }\vb{allele frequencies} \DIFadd{are
selection, which systematically increases beneficial alleles and removes
}\vb{deleterious} \DIFadd{ones, and genetic drift which stochastically changes allele
...
\DIFadd{quantify }\vb{the relative contributions of} \DIFadd{selection and drift }\vb{to} \DIFadd{allele
frequency }\vb{changes} \DIFadd{at a genome-wide level through time. We show selection acts
over short timescales in three evolve-and-resequence studies and has a sizeable
impact on genome-wide allele frequency changes. 
}
...
drift. \DIFdelbegin \DIFdel{We estimate the lower bound on the proportion of total variation in
...
short timescales.
}\DIFdelend \DIFaddbegin \vb{We estimate a lower bound of the fraction of variance in allele
  frequency change caused by selection, as well as the correlation between
  allele frequency changes between replicate populations caused by convergent
selection pressures. Overall, we demonstrate that linked selection has a
powerful role in shaping genome-wide allele frequency changes over very short
timescales.}
\DIFaddend 
...
sequenced every ten generations, in the numerator for the covariance we use the
allele frequency changes between adjacent timepoints, which are ten generations
apart\DIFaddbegin \DIFadd{; }\vb{thus the strong covariances between adjacent generations are not
  observable and do not contribute to the numerator of $G(t)$}\DIFadd{.
  }\vb{Additionally, selection also inflates the variance in allele frequency
    change per generation; however, this effect cannot be easily distinguished
  from drift.} \vb{For both these reasons, our measure $G(t)$ is strongly
conservative} \DIFadd{(}\vb{we demonstrate this through simulations in Supplementary
Material Section \ref{supp:tempblocks}}\DIFadd{)}\DIFaddend .  \DIFdelbegin \DIFdel{Consequently, this leads our measure $G(t)$ to
...
the numerator of $G(t)$. }\DIFdelend Still, we find a remarkably strong
signal. Greater than $20\%$ of total, genome-wide allele frequency change over
60 generations is the result of selection (Figure \ref{fig:figure-1} B). 
...
is 0.18, 95\% CI }[\DIFadd{0.0743, 0.254}]\DIFadd{).
}\DIFaddend 
...
the first two rows in Figure \ref{fig:figure-1} A). This shows that alleles
that were on average going up initially are later going down in frequency, i.e.
that the average direction of selection experienced by alleles has flipped.
This must reflect either a change in the environment or the genetic background,
due to epistatic relationships among alleles altered by frequency changes or
recombination breaking up selective alleles.  Such reversals of selective
...
\ref{fig:figure-3} A and B) that \DIFdelbegin \DIFdel{windowed temporal }\DIFdelend \DIFaddbegin \DIFadd{there are an empirical excess of windows with
positive }\DIFaddend covariances between close timepoints \DIFdelbegin \DIFdel{are skewed positive (a heavy }\DIFdelend \DIFaddbegin \DIFadd{compared to the null distribution
(a heavier }\DIFaddend right tail), \DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{and that this then shifts to an excess of windows with
negative covariances }\DIFaddend between more distant timepoints \DIFdelbegin \DIFdel{these windowed temporal covariances tend to shift to become more
...
of varying magnitudes (Figure \ref{fig:figure-4}A)}\DIFadd{. We find that positive
...
\DIFadd{These }\vb{simulation} \DIFadd{results are often relatively insensitive to the number of
...
selection response we see in }\vb{the \textcite{Barghi2019-qy} data} \DIFadd{is highly
...
\ref{suppfig:supp-bgs-covs}) }\vb{and under some circumstances, can even
  ...
  \ref{suppfig:supp-bgs-converg})}\DIFadd{.  }\vb{Thus it is unlikely that the
signatures of linked selection we see are entirely} \DIFadd{the result of the novel
...
experiment, does the control line }\vb{allow us to conclude that} \DIFadd{selection
...
frequency }\vb{changes}\DIFadd{.
}

\DIFaddend \section{Discussion}

Since the seminal analysis of \textcite{Maynard_Smith1974-lc} demonstrating
that linked neutral diversity is reduced as an advantageous polymorphism
\DIFdelbegin \DIFdel{arises
and }\DIFdelend \DIFaddbegin \vbout{arises and} \DIFaddend sweeps to fixation, over four decades of theoretical and
empirical research has \DIFdelbegin \DIFdel{enriched }\DIFdelend \DIFaddbegin \vb{bettered} \DIFaddend our understanding of linked selection.
One under-used approach to understand the genome-wide effects of selection on
\DIFdelbegin \DIFdel{standing variation, e.g.
selection on an infinitesimal polygenic trait, stems }\DIFdelend \DIFaddbegin \vb{polygenic trait (e.g. on standing variation, stems} \DIFaddend from an early
quantitative genetic model of linked selection \parencite{Robertson1961-ho} and
...
(Supplementary Figure \ref{suppfig:bergland-correction}\DIFdelbegin \DIFdel{, XXX).  }\DIFdelend \DIFaddbegin \DIFadd{).  Through our
...
After a $10N$ burnin, the population was split into two different replicate
population sizes, to capture the effect of bottlenecks in selection experiments
(these population sizes were, 50, 500, and 1000 diploids; the later
representing no bottleneck). Each population then underwent an optima shift of
either 0.1, 0.5, or 1 on generation five, with the first four generations
serving as a control. These optima shifts were either in the same direction
(converging), different directions (diverging), or only one optima shifted (as
a control). By tracking the trait mean, we saw that it converged as expected
during burnin, and had the appropriate response to selection (Supplementary
Material Figure \ref{suppfig:gss-zbar}). Using the neutral population frequency
data from these simulations, we calculated the temporal covariances, $G(t)$
trajectories, and convergence correlations.
}
...
follow that of seasonal fluctuating selection\DIFaddbegin \DIFadd{.  
}
