...
def temporal_cov(freqs, depths=None, diploids=None, *args, **kwargs):
    """
    Notes:
    ...
    R, T, L = deltas.shape
    flat_deltas = flatten_matrix(deltas, R, T, L)
    if depths is not None:
        depths = flatten_matrix(depths, R, T+1, L)
    if diploids is not None:
        diploids = diploids.reshape((1, R*(T+1)))
    return corrected_cov(flat_deltas,
                         mean_hets,
                         depths=depths,
                         diploids=diploids, **kwargs)


def replicate_averaged_hets(hets, R, T, L):
    """
    ...
    half_hets = hets[:,:-1] / 2
    assert(half_hets.shape[0] == R)
    assert(half_hets.shape[1] == T)
    # Create the heterozygosity denominator, which is of block form.
    # Each block is a replicate. Each element is,
    # (p_{A, min(t,s)}(1-p_{A, min(t,s)}) +
    #      p_{B, min(t,s)}(1-p_{B, min(t,s)})) / 2
    ...
    A = het_denom[row_bm.ravel(), min_t_s_block.ravel()]
    B = het_denom[col_bm.ravel(), min_t_s_block.ravel()]
    avehet_min = (A + B).reshape((R*T, R*T)) / 2
    #avehet_min = het_denom[i.ravel(), j.ravel()].reshape((R * T, R * T))
