...
\begin{abstract}
 ...
 change \gc{in allele} frequency at one timepoint is predictive of the
 changes at later timepoints when there is heritable fitness variation in the
 population, \gc{as alleles} can remain associated with selected alleles
 over time \gc{and so be indirectly selected for}. Since genetic drift does not lead to temporal covariance, we can
...
of linked \gc{alleles}; in contrast, when genetic drift acts alone in a
closed population, these covariances are zero in expectation \gc{for
  neutral alleles}.  Mathematically,
...
\gc{allele} frequency changes in one generation predict changes in another
...
linkage disequilibrium and the \gc{associated} additive genetic
variance for fitness decays (\vb{which could occur as a population reaches
...
covariances: \gc{alleles} linked to a particular fitness background are
...
reflect that \gc{alleles} associated with heritable fitness backgrounds are
...
\gc{were maintained at relatively small census sizes (Table
\ref{table:study_summary})}, which will amplify the role of genetic drift. The
advantage of lab experiments is that they are closed populations\vb{;} in
...
To help help reconcile our measure of linked selection, $G(t)$, with familiar
reductions  \gc{in neutral diversity, as parameterized by} $N_e$, we develop some rough
...
where $\varepsilon \ge 0$ since these variances must be positive.

\gc{In a neutral} Wright--Fisher population, 
