...
%DIF DEL old.tex          Thu May 14 10:13:08 2020
%DIF ADD manuscript.tex   Thu May 14 13:15:09 2020
...
\RequirePackage[bibstyle=authoryear,citestyle=authoryear-comp,
                date=year,
%DIF 29c29
%DIF <                 maxbibnames=9,maxnames=5,maxcitenames=2,
%DIF -------
                maxbibnames=99,maxnames=99,maxcitenames=2, %DIF > 
%DIF -------
...
%DIF < % - sig impact
%DIF < % - ref table 1
%DIF < % - more correction plots?
%DIF -------
% - divide through the temp block supp. figure by combinatorics %DIF > 
%DIF -------
% - mention PCA
...
\begin{abstract}
 ...
 change in neutral allele
   frequency }\DIFdelend \DIFaddbegin \DIFadd{heritable fitness variation in the population leads the changes in allele
   frequencies }\DIFaddend at one timepoint \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{to be }\DIFaddend predictive of the changes at later
 timepoints\DIFdelbegin \DIFdel{when there is heritable fitness variation in the
 population, as neutral alleles can remain associated }\DIFdelend \DIFaddbegin \DIFadd{, as alleles are indirectly selected due to remaining
 associations }\DIFaddend with selected alleles\DIFdelbegin \DIFdel{over time}\DIFdelend . Since genetic drift does not lead to
 ...
 selection. Here, we reanalyze \DIFdelbegin \DIFdel{two }\emph{\DIFdel{Drosophila simulans}} %DIFAUXCMD
\DIFdel{evolve-and-resequence studies, and
 one artificial selection experiment in mice, }\DIFdelend \DIFaddbegin \DIFadd{three selection experiments }\DIFaddend to quantify the
...
\DIFaddbegin \subsubsection*{\DIFadd{Significant Statement}}

\DIFadd{A long-standing problem in evolution biology is to understand the different
processes that shape the genetic composition of populations. In a population
...
Selection can \DIFaddbegin \DIFadd{affect allele frequencies, }\DIFaddend both directly and indirectly\DIFdelbegin \DIFdel{affect allele frequencies}\DIFdelend , with the
indirect effect coming from the action of selection on correlated loci
elsewhere in genome\DIFaddbegin \DIFadd{, }\DIFaddend e.g. linked selection (\cite{Maynard_Smith1974-zr},
\cite{Charlesworth1993-gb,Nordborg1996-nq}; see \cite{Barton2000-zg} for a
review). Previous work \DIFdelbegin \DIFdel{on this question }\DIFdelend has mostly focused on teasing apart the impacts of drift
...
However, we lack an understanding of \DIFdelbegin \DIFdel{how genome-wide }\DIFdelend \DIFaddbegin \DIFadd{both how }\DIFaddend linked selection acts over \DIFdelbegin \DIFdel{time}\DIFdelend \DIFaddbegin \DIFadd{short
time intervals and its full impact on genome-wide allele frequency changes}\DIFaddend .
...
\DIFaddbegin \parencite{Latta1998-me, Pritchard2010-tk,Kemper2014-bx}\DIFaddend . This is because the
effect of selection on a polygenic trait (such as fitness) is distributed
across \DIFdelbegin \DIFdel{loci in proportion to their effect sizes}\DIFdelend \DIFaddbegin \DIFadd{numerous loci}\DIFaddend . This can lead to subtle allele frequency shifts on
standing variation that are difficult to distinguish from background levels of
genetic drift and sampling variance. \DIFdelbegin \DIFdel{However, increasingly }\DIFdelend \DIFaddbegin \DIFadd{Increasingly, }\DIFaddend genomic experimental
evolution studies with multiple timepoints, and in some cases multiple
replicate populations, are being used to detect \DIFdelbegin \DIFdel{large effect }\DIFdelend \DIFaddbegin \DIFadd{large-effect }\DIFaddend selected loci
...
polygenic linked selection detectable from \DIFdelbegin \DIFdel{genomic studies with multiple
timepoints }\DIFdelend \DIFaddbegin \DIFadd{temporal genomic data }\DIFaddend is the
temporal autocovariance \DIFaddbegin \DIFadd{(i.e. covariance between two timepoints) }\DIFaddend in allele
frequency changes \parencite{Buffalo2019-io}.  These covariances are \DIFdelbegin \DIFdel{directly estimable from
temporal genomic data and are }\DIFdelend created
...
population, these covariances are \DIFdelbegin \DIFdel{zero in expectation}\DIFdelend \DIFaddbegin \DIFadd{expected to be zero for neutral alleles}\DIFaddend .
Mathematically, temporal covariances are useful because it is natural to
decompose the total variance in allele frequency change across a \DIFdelbegin \DIFdel{set of time intervals }\DIFdelend \DIFaddbegin \DIFadd{time interval
}\DIFaddend into the variances and covariances in allele frequency change \DIFdelbegin \DIFdel{among time intervals}\DIFdelend \DIFaddbegin \DIFadd{between
generations}\DIFaddend .  Furthermore, biologically, these covariances reflect the extent
to which \DIFdelbegin \DIFdel{neutral }\DIFdelend allele frequency changes in one generation predict changes in another
...
artificial selection experiment \parencite{Castro2019-uk}. \DIFaddbegin \DIFadd{These sequencing
...
frequency genome-wide is driven by selection or genetic drift. }\DIFaddend We repeatedly
...
drift. We estimate \DIFdelbegin \DIFdel{the lower bound on the proportion of total variation }\DIFdelend \DIFaddbegin \DIFadd{a lower bound of the fraction of variance }\DIFaddend in allele
frequency change caused by selection, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{as well as }\DIFaddend the correlation between allele
frequency changes between replicate populations caused by \DIFdelbegin \DIFdel{the response to
}\DIFdelend convergent selection
...
\begin{table}
  ...
  \begin{tabular}{l c c c c c c}
    \textbf{Study} & \textbf{Species} & \textbf{Selection} & \textbf{Replicates} & \textbf{Pop. Size}\DIFaddbeginFL \DIFaddFL{$^{\dagger}$ }\DIFaddendFL & \textbf{Gens.} & \textbf{Timepoints} \\ 
  \hline
 ...
 \hline
  ...
  \caption{A summary of the main selection studies we
    analyzed. \DIFaddbeginFL \DIFaddFL{$^{\dagger}$Approximate census population size
      during experiment.}\DIFaddendFL } 
      \DIFaddbeginFL \label{table:study_summary} 
...
change (in ten-generation intervals, denoted $\Delta_{_{10}} p_t$) \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend some
...
biases created due to sampling noise, and \DIFdelbegin \DIFdel{normalize }\DIFdelend \DIFaddbegin \DIFadd{normalized }\DIFaddend the entries for
...
with neutral alleles decays }\DIFdelend \DIFaddbegin \DIFadd{decays (which could occur as a population reaches a new optimum)
...
\DIFdelbegin \DIFdel{\textcite{Barghi2019-qy}'s design means that the covariances we see in adjacent
...
large-effect loci rather than selection on a polygenic trait. }\DIFdelend Since our covariances are \DIFdelbegin \DIFdel{essentially }\DIFdelend averages over loci, the covariance estimate \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend be
strongly affected by a \DIFdelbegin \DIFdel{single outlier region}\DIFdelend \DIFaddbegin \DIFadd{few outlier regions}\DIFaddend . To test whether large outlier
...
the impact of selection acting across many genomic \DIFdelbegin \DIFdel{windows}\DIFdelend \DIFaddbegin \DIFadd{regions}\DIFaddend . 


\begin{figure}[!htb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/figure-1.pdf}
%DIFDELCMD < %%%
...
variance and covariance components, \DIFdelbegin \DIFdel{$\var(p_t - p_0) = \sum_{i=0}^{t-1}
\var(\Delta p_i) + \sum_{i \ne j}^{t-1} \cov(\Delta p_i, \Delta p_j)$ (we bias
correct }\DIFdelend \DIFaddbegin \DIFadd{$\var(p_t - p_0) = \sum_{i=0}^{t-1}
  \var(\Delta p_i) + \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i,
\Delta p_j)$ (we bias-correct }\DIFaddend these for sequencing depth), and the
...
\begin{align}
  G(t) = \DIFdelbegin \DIFdel{\frac{\sum_{i\ne j}^{t-1} \cov(\Delta p_i, \Delta p_j)}{\var(p_t - p_0)}
}\DIFdelend \DIFaddbegin \DIFadd{\frac{ \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i, \Delta p_j)}{\var(p_t - p_0)}
}\DIFaddend \end{align}
%
which estimates the \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{impact }\DIFaddend of selection on allele frequency change between
the initial generation $0$ and some later generation $t$, which can be varied
to see how this quantity grows through time. \DIFaddbegin \DIFadd{When the sum of the covariances is
...
\ref{supp:reduction-approx}). }\DIFaddend Since \textcite{Barghi2019-qy} experiment is
sequenced every ten generations, \DIFdelbegin \DIFdel{in the numerator for the covariance we use the allele frequency changes between adjacent timepoints,
which are ten generations apart. Consequently, this leads }\DIFdelend \DIFaddbegin \DIFadd{the numerator uses the covariances
...
cannot be easily distinguished from drift. For both these reasons, }\DIFaddend our measure
$G(t)$ \DIFdelbegin \DIFdel{to
be strongly conservative , since the temporal covariances within each
ten-generation block are not directly observable, and thus are not included in the numerator of $G(t)$}\DIFdelend \DIFaddbegin \DIFadd{is considerably conservative (we demonstrate this through
simulations in Supplementary Material Section \ref{supp:tempblocks})}\DIFaddend .  Still,
...
\ref{fig:figure-1} B). \DIFaddbegin \DIFadd{This proportion of variance attributable to selection
...
However, we find no such signal over years\DIFaddbegin \DIFadd{, and in reproducing their original
...
seasonal labels}\DIFaddend ; we discuss this in more depth in Supplementary Materials
...
Figure \ref{fig:figure-2}A). We find these convergent \DIFdelbegin \DIFdel{covariances }\DIFdelend \DIFaddbegin \DIFadd{correlation coefficients
}\DIFaddend are relatively weak, and decay very quickly from an initial value of about 0.1
...
\DIFdelbegin \DIFdel{reasonable }\DIFdelend \DIFaddbegin \DIFadd{substantial }\DIFaddend fraction of the initial response is shared over the replicates,
...
\begin{figure}[!htb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/figure-2.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figures/figure-2-revision.pdf}
\DIFaddendFL 

  \caption{{\bf A}: The convergence \DIFdelbeginFL \DIFdelFL{correlation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correlations}\DIFaddendFL , averaged across
    ...
    convergence correlation matrix depicted to the right \DIFaddbeginFL \DIFaddFL{(where the
      ...
      replicates. {\bf B}: The convergence \DIFdelbeginFL \DIFdelFL{covariance }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correlations }\DIFaddendFL between individual
      pairs of replicates in the \textcite{Kelly2019-dc} data \DIFaddbeginFL \DIFaddFL{(note the
      confidence intervals are plotted, but are small on this y-axis scale)}\DIFaddendFL .
      {\bf C}:  The convergence \DIFdelbeginFL \DIFdelFL{covariance }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{correlations }\DIFaddendFL between individual pairs of
      ...
      selection between LS1 and LS2. \DIFaddbeginFL \DIFaddFL{Through simulations, we have found
        that the differences in convergence correlation confidence interval
        widths between these }\emph{\DIFaddFL{Drosophila}} \DIFaddFL{studies and the Longshanks study
...
positive \DIFdelbegin \DIFdel{between-replicate covariances}\DIFdelend \DIFaddbegin \DIFadd{convergence correlations}\DIFaddend .  We find all three \DIFdelbegin \DIFdel{pairwise
between-replicate covariances are positive and statistically }\DIFdelend \DIFaddbegin \DIFadd{convergent
correlation coefficients between replicate pairs are }\DIFaddend significant (Figure
\ref{fig:figure-2}B\DIFdelbegin \DIFdel{. We estimate the convergent correlation
coefficient across these replicates as }\DIFdelend \DIFaddbegin \DIFadd{), and average to }\DIFaddend 0.36 ($95\%$ CI $[0.31, 0.40]$).
\DIFdelbegin \DIFdel{Similarly}\DIFdelend \DIFaddbegin \DIFadd{Additionally}\DIFaddend , we can calculate the proportion of the total variance in
allele frequency change from convergent selection pressure\DIFdelbegin \DIFdel{analogous to $G$ }\DIFdelend \DIFaddbegin \DIFadd{, analogous to
our $G(t)$, }\DIFaddend where the numerator is \DIFdelbegin \DIFdel{is }\DIFdelend the convergent covariance and the
...
line (Ctrl) \DIFaddbegin \DIFadd{where parents were randomly selected}\DIFaddend . Consequently, this selection
experiment offers a useful control to test our \DIFdelbegin \DIFdel{between-replicate covariances}\DIFdelend \DIFaddbegin \DIFadd{convergence correlations}\DIFaddend : we
expect to see \DIFdelbegin \DIFdel{positive
between-replicate covariance }\DIFdelend \DIFaddbegin \DIFadd{significant positive convergence correlations }\DIFaddend in the comparison
between the two Longshanks selection lines, but not between \DIFdelbegin \DIFdel{the two pairwise comparisons between the
}\DIFdelend \DIFaddbegin \DIFadd{each of the }\DIFaddend control
line and \DIFdelbegin \DIFdel{each of the two Longshanks lines}\DIFdelend \DIFaddbegin \DIFadd{Longshanks line pairs}\DIFaddend . We find that this is the case (gray confidence
intervals in Figure \ref{fig:figure-2}C), with \DIFdelbegin \DIFdel{the
two Longshanks comparisons }\DIFdelend \DIFaddbegin \DIFadd{convergence correlations between
each of the Longshanks lines }\DIFaddend to the control \DIFdelbegin \DIFdel{line not being significantly
}\DIFdelend \DIFaddbegin \DIFadd{not being statistically }\DIFaddend different
from zero, while the \DIFdelbegin \DIFdel{comparison }\DIFdelend \DIFaddbegin \DIFadd{convergence correlation }\DIFaddend between the two Longshanks lines
is \DIFdelbegin \DIFdel{statistically significantly different from zero }\DIFdelend \DIFaddbegin \DIFadd{strong (0.18) and statistically significant }\DIFaddend (CIs [\DIFdelbegin \DIFdel{0.0129, 0.0400}\DIFdelend \DIFaddbegin \DIFadd{0.07, 0.25}\DIFaddend ]).

One finding in the Longshanks study was that two major-effect loci showed
parallel frequency shifts between the two selection lines\DIFdelbegin \DIFdel{: a region harboring
the gene Nkx3-2 known to be involved in limb development, and another region
containing six other candidate genes}\DIFdelend . We were curious to
...
\DIFdelbegin %DIFDELCMD < \vb{The presence of an unselected control line provides an alternative way to
%DIFDELCMD < partition the effects of linked selection and genetic drift: we can compare the
%DIFDELCMD < total variance in allele frequency change of the control line (which excludes
%DIFDELCMD < the effect of artificial selection on allele frequencies) to the total variance
%DIFDELCMD < in frequency change of the Longshanks selection lines. This allows us to
%DIFDELCMD < estimate the increase in variance in allele frequency change due to selection,
%DIFDELCMD < which we can further partition into the effects of selection shared between
%DIFDELCMD < selection lines and those unique to a selection line by using estimating the
%DIFDELCMD < shared effect through the observed covariance between replicates (see Materials
%DIFDELCMD < and Methods \ref{sec:mm-partition} and Supplementary Material Section
%DIFDELCMD < \ref{supp:replicate-g} for more details).  We estimate at least 32\% (95\% CI
%DIFDELCMD < $[21\%, 48\%]$) of the variance in allele frequency change is driven by the
%DIFDELCMD < effects of selection, of which 14\% (95\% CI $[3\%, 33\%]$) is estimated to be
%DIFDELCMD < unique to a selection line, and 17\% (95\% CI $[9\%, 23\%]$) is the effect of
%DIFDELCMD < shared selection between the two Longshanks selection lines (and the value of
%DIFDELCMD < the convergence correlation between the Longshanks lines, a related statistic,
%DIFDELCMD < is 0.18, 95\% CI [0.0743, 0.254]).}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The presence of an unselected control line provides an alternative way to
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[]{figures/figure-3.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[]{figures/figure-3-revision.pdf}
\DIFaddendFL 
...
\DIFdelbegin \DIFdel{Finally, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend observed that in the longest study  we analyzed \parencite{Barghi2019-qy},
...
This \DIFdelbegin \DIFdel{must }\DIFdelend \DIFaddbegin \DIFadd{might }\DIFaddend reflect either a change in the environment or the genetic
background, due to epistatic relationships among alleles altered by frequency
changes \DIFaddbegin \DIFadd{(which can occur during an optima shift; \mbox{%DIFAUXCMD
\cite{Hayward2019-kt}}\hspace{0pt}%DIFAUXCMD
) }\DIFaddend or
recombination breaking up selective alleles. Such reversals \DIFdelbegin \DIFdel{of selective }\DIFdelend \DIFaddbegin \DIFadd{in selection
}\DIFaddend dynamics could be occurring at other timepoints but the signal of a change in
...
due to linkage disequilibria\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend which can occur over long physical distances in
E\&R and selection studies \DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\cite{Nuzhdin2013-gf,Baldwin-Brown2014-cl}}\hspace{0pt}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \parencite{Nuzhdin2013-gf,Baldwin-Brown2014-cl}\DIFaddend .  To
address this, we have developed a permutation-based procedure that constructs
an empirical \DIFaddbegin \DIFadd{neutral }\DIFaddend null distribution by randomly flipping the \DIFdelbegin \DIFdel{signs }\DIFdelend \DIFaddbegin \DIFadd{sign }\DIFaddend of
the allele frequency changes \DIFdelbegin \DIFdel{per-genomic window . }\DIFdelend \DIFaddbegin \DIFadd{in each genomic window (i.e. a single random sign
flip is applied to all loci in a window). }\DIFaddend This destroys the systematic
...
drift alone\DIFdelbegin \DIFdel{as they
include the effect of selection on the }\DIFdelend \DIFaddbegin \DIFadd{, as selection not only creates covariance between time
intervals, but also inflates the magnitude of }\DIFaddend allele frequency change within a
time-interval\DIFdelbegin \DIFdel{, just not between time-intervals}\DIFdelend .  We see (Figure \ref{fig:figure-3} A and B) that \DIFdelbegin \DIFdel{windowed temporal }\DIFdelend \DIFaddbegin \DIFadd{there are an
empirical excess of windows with positive }\DIFaddend covariances between close timepoints
\DIFdelbegin \DIFdel{are skewed positive (a heavy }\DIFdelend \DIFaddbegin \DIFadd{compared to the null distribution (a heavier }\DIFaddend right tail), \DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{and that this then
shifts to an excess of windows with negative covariances }\DIFaddend between more distant
timepoints \DIFdelbegin \DIFdel{these windowed temporal covariances tend to shift to become more
negative (a heavy }\DIFdelend \DIFaddbegin \DIFadd{(a heavier }\DIFaddend left tail)
\DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin 

\DIFaddend We quantified the degree to which the left and
...
\DIFaddbegin \begin{figure}[!htb]
  ...
  \caption{\DIFaddFL{Forward-in-time simulations of demonstrate how temporal covariance,
    ...
    (}{\bf \DIFaddFL{A}}\DIFaddFL{) Trait means across 30 replicate before and after optima shifts (solid
    lines), for two different magnitudes (indicated by color). The optimal
    trait values are indicated by the purple and yellow dashed lines.  (}{\bf \DIFaddFL{B}}\DIFaddFL{) Mean
    temporal covariance $\cov(\Delta p_5, \Delta p_t)$ across 30 simulation
    replicates, where $t$ varies along the x-axis (points), with a
    loess-smoothed average (solid line). (}{\bf \DIFaddFL{C}}\DIFaddFL{) $G(t)$ trajectories through time,
    for 30 replicate simulations across two optima shifts. The solid line is a
    loess-smoothed average. (}{\bf \DIFaddFL{D}}\DIFaddFL{) The convergence correlations between two
    ...
    opposite directions (diverge) at generation five. In subfigures (}{\bf \DIFaddFL{B}}\DIFaddFL{), (}{\bf \DIFaddFL{C}}\DIFaddFL{),
    and (}{\bf \DIFaddFL{D}}\DIFaddFL{), directional selection begins at generation five, when the optima
    shifts; this is indicated by the vertical dashed red line.}}

  \label{fig:figure-4}
\end{figure}

\DIFadd{Finally we used forward-in-time simulations to explore the conditions under
...
\DIFadd{These simulation results are often relatively insensitive to the number of loci
...
\DIFadd{Additionally, we explored other selection schemes. We find that the long term
...
\DIFadd{While none of our experiments have selected the populations in divergent
...
that linked neutral diversity is reduced as an advantageous polymorphism \DIFdelbegin \DIFdel{arises
and }\DIFdelend sweeps
to fixation, over four decades of theoretical and empirical research has
\DIFdelbegin \DIFdel{enriched }\DIFdelend \DIFaddbegin \DIFadd{bettered }\DIFaddend our understanding of linked selection.  One under-used approach to
understand the genome-wide effects of selection on \DIFdelbegin \DIFdel{standing variation, }\DIFdelend \DIFaddbegin \DIFadd{polygenic trait (}\DIFaddend e.g.  \DIFdelbegin \DIFdel{selection on
an infinitesimal polygenic trait}\DIFdelend \DIFaddbegin \DIFadd{on
standing variation}\DIFaddend , stems from an early quantitative genetic model of linked
...
\parencite{Buffalo2019-io}.  Depending on how many loci affect fitness, \DIFdelbegin \DIFdel{such }\DIFdelend \DIFaddbegin \DIFadd{even }\DIFaddend a
strong effect of linked selection may not be differentiable from genetic drift
using only single contemporary population samples or looking at \DIFdelbegin \DIFdel{at }\DIFdelend temporal allele
...
these populations is due to the (\DIFaddbegin \DIFadd{likely }\DIFaddend indirect) action of selection.
...
across replicate populations, and found that a \DIFdelbegin \DIFdel{reasonable }\DIFdelend \DIFaddbegin \DIFadd{substantial }\DIFaddend proportion of
...
be seen as a strong lower bound of the effects of selection\DIFaddbegin \DIFadd{, which we have
...
patterns of \DIFdelbegin \DIFdel{LD }\DIFdelend \DIFaddbegin \DIFadd{linkage disequilibria (LD) }\DIFaddend and recombination which would allow
...
such as the additive genetic \DIFdelbegin \DIFdel{variation }\DIFdelend \DIFaddbegin \DIFadd{variance }\DIFaddend for fitness directly from temporal
genomic data alone \parencite{Buffalo2019-io}.

%DIF > very small effective population sizes, estimated
%DIF > at 300, 450, and 45 for the \textcite{Barghi2019-qy}, \textcite{Kelly2019-dc},
%DIF > and \textcite{Castro2019-uk} studies respectively
\DIFaddbegin 
...
stronger or more sustained \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend in natural populations
...
closed populations\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend in natural populations temporal covariance could also
...
timescales that linked selection \DIFdelbegin \DIFdel{act over }\DIFdelend \DIFaddbegin \DIFadd{acts over in the wild}\DIFaddend . In our
...
\DIFdelbegin \DIFdel{only from }\DIFdelend \DIFaddbegin \DIFadd{from only }\DIFaddend contemporary populations. Thus, we can hope to have a much
...
\textcite{Kelly2019-dc}, \DIFaddbegin \DIFadd{and }\DIFaddend \textcite{Bergland2014-ij}, and
\DIFaddbegin \DIFadd{individual-level sequencing data from }\DIFaddend \textcite{Castro2019-uk}. In all cases,
...
the reference or minor allele, we randomly choose \DIFdelbegin \DIFdel{which allele 's frequency to track
}\DIFdelend \DIFaddbegin \DIFadd{an allele to track
frequency }\DIFaddend for each locus. Then, we calculate the variance-covariance
...
covariances (Supplementary Figure \ref{suppfig:bergland-correction}\DIFdelbegin \DIFdel{, XXX).  }\DIFdelend \DIFaddbegin \DIFadd{).  Through
...
resamples blocks of \DIFdelbegin \DIFdel{data points}\DIFdelend \DIFaddbegin \DIFadd{loci}\DIFaddend , rather than individual \DIFdelbegin \DIFdel{data points}\DIFdelend \DIFaddbegin \DIFadd{loci}\DIFaddend , to infer the
uncertainty of \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend statistic in the presence of unknown correlation between loci.
As most estimators in this paper are ratios (e.g. covariance standardize by
sample heterozygosity, $G(t)$, and the convergence correlation)\DIFaddbegin \DIFadd{, }\DIFaddend which we
estimate with a ratio of averages, we exploit the linearity of expectation for
efficient computation of bootstrap samples (see \DIFaddbegin \DIFadd{Supplementary Material Section
}\DIFaddend \ref{supp:block-bootstrap} for details).

\subsection{Partitioning Unique and Shared Selection Effects in the Longshanks Study}
\label{sec:mm-partition}

\DIFdelbegin %DIFDELCMD < \vb{The unselected control line in the Longshanks experiment allows us to
%DIFDELCMD < additionally partition the total variance in allele frequency change into
%DIFDELCMD < drift, shared effects of selection, and unshared effects of selection between
%DIFDELCMD < selected replicates. We begin by decomposing the allele frequency change in
%DIFDELCMD < Longshanks line 1 (LS1) as $\Delta p_{t,\mathrm{LS1}} = \Delta_{_{D}}
%DIFDELCMD < p_{t,\mathrm{LS1}} + \Delta_{_{U}} p_{t,\mathrm{LS1}} + \Delta_{_S}
%DIFDELCMD < p_{t,\mathrm{LS}}$ where these terms are the drift in Longshanks replicate 1
%DIFDELCMD < ($\Delta_{_D} p_{t,\mathrm{LS1}}$), selection unique to the LS1 replicate
%DIFDELCMD < ($\Delta_{_U} p_{t,\mathrm{LS1}}$), and selection response shared between the
%DIFDELCMD < two Longshanks replicates ($\Delta_{_S} p_{t,\mathrm{LS}}$) respectively (and
%DIFDELCMD < similarly for the Longshanks line 2, LS2). By construction, this decomposition
%DIFDELCMD < assumes that each of these terms are uncorrelated within replicates, so the
%DIFDELCMD < contribution of each term to the total variance in allele frequency change,
%DIFDELCMD < $\var(\Delta p_{t,\mathrm{LS1}})$, is the variance in of that term's allele
%DIFDELCMD < frequency change. 
%DIFDELCMD < 

%DIFDELCMD < We estimate the effects of selection by first calculating the fraction of the
%DIFDELCMD < total variance explained by drift. We assume the variance in allele frequency
%DIFDELCMD < change observed in the unselected control line ($\var(\Delta
%DIFDELCMD < p_{t,\mathrm{Ctrl}})$) is driven entirely by neutral genetic drift, and since
%DIFDELCMD < an identical breeding scheme was used across all three replicates (except
%DIFDELCMD < breeders for the control line were chosen at random), we can use this as an
%DIFDELCMD < estimate of the contribution of neutral genetic drift in the selected lines,
%DIFDELCMD < $\var(\Delta p_{t,\mathrm{Ctrl}}) = \var(\Delta_{_{D}} p_{t,\mathrm{LS1}}) =
%DIFDELCMD < \var(\Delta_{_{D}} p_{t,\mathrm{LS2}})$. Then, we can estimate the increase in
%DIFDELCMD < variance in allele frequency change due to selection as $(\var(\Delta
%DIFDELCMD < p_{t,\mathrm{LS1}}) + \var(\Delta p_{t,\mathrm{LS2}}))/2 - \var(\Delta
%DIFDELCMD < p_{t,\mathrm{Ctrl}})$ and the shared effect of selection across selected lines
%DIFDELCMD < as $\cov(\Delta p_{t,\mathrm{LS1}}, \Delta p_{t,\mathrm{LS2}})$. Finally, the
%DIFDELCMD < covariance in allele change between replicates is used to estimate the shared
%DIFDELCMD < effects of selection between lines, $\cov(\Delta p_{t,\mathrm{LS1}}, \Delta
%DIFDELCMD < p_{t,\mathrm{LS2}}) = \var(\Delta_{_S} p_{t,\mathrm{LS}})$.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The unselected control line in the Longshanks experiment allows us to
...
\DIFaddbegin \DIFadd{We estimate the effects of selection by first calculating the fraction of the
...
\DIFaddend \subsection{Windowed Covariance and the Empirical Neutral Null}
...
\DIFdelbegin %DIFDELCMD < \vb{Given evidence of a reversal in the direction of selection at later timepoints
%DIFDELCMD < in the \textcite{Barghi2019-qy} study, we calculated windowed temporal
%DIFDELCMD < covariances on 10 kilobase windows and looked at the distribution of these
%DIFDELCMD < covariances through time. We compare these distributions of windowed
%DIFDELCMD < covariances to an empirical neutral null created by randomly permuting the sign
%DIFDELCMD < of allele frequency change at the block level (to preserve the correlation
%DIFDELCMD < structure between loci due to LD). This destroys the systematic covariances in
%DIFDELCMD < allele frequency change created by linked selection, which emulates a frequency
%DIFDELCMD < trajectory under drift. This approach is conservative, since heritable fitness
%DIFDELCMD < variation also inflates the magnitude of allele frequency change more than
%DIFDELCMD < expected under drift, but we do not change these magnitudes. Using this
%DIFDELCMD < empirical neutral null distribution of windowed covariances, we calculate how
%DIFDELCMD < much of the observed windowed covariance distribution falls outside of
%DIFDELCMD < empirical null distribution for different tail probabilities. While the
%DIFDELCMD < comparison between the distribution of 10 kilobase windowed covariances to the
%DIFDELCMD < empirical neutral null created from sign-permuting 10 kilobase windows is most
%DIFDELCMD < natural, we wanted to ensure that our finding that the shift from mostly
%DIFDELCMD < positive to mostly negative windowed covariances through time (Figure
%DIFDELCMD < \ref{fig:figure-3}) was robust to LD extending beyond the range of these 10
%DIFDELCMD < kilobase windows. We took the conservative approach of also sign-permuting at
%DIFDELCMD < the chromosome-level, and found the same qualitative shift (Supplementary
%DIFDELCMD < Figure \ref{suppfig:barghi-tailprobs-seqid}).}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Given evidence of a reversal in the direction of selection at later timepoints
...
\DIFaddbegin \subsection{\DIFadd{Forward-in-time Simulations}}

\DIFadd{To explore how aspects of genetic architecture, model of selection, and
...
}\parencite{Haller2019-vu}\DIFadd{; here we discuss the Gaussian Stabilizing Selection
...
\DIFadd{We simulated directional selection on a trait by first burning in a population
...
megabases in length (about one quarter of a }\emph{\DIFadd{Drosophila}} \DIFadd{chromosome), and
...
We would like to thank the authors of the original studies we \DIFdelbegin \DIFdel{'ve }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend analyzed,
including Neda Barghi, \DIFdelbegin \DIFdel{Christian Schl}%DIFDELCMD < {%%%
\DIFdel{\"o}%DIFDELCMD < }%%%
\DIFdel{tterer, John Kelly, Kimberly Hughes,
}\DIFdelend \DIFaddbegin \DIFadd{Nick Barton, Alan Bergland, }\DIFaddend Frank Chan, \DIFdelbegin \DIFdel{Campbell Rolian,
Nick Barton, Alan Bergland, and Dmitri Petrov}\DIFdelend \DIFaddbegin \DIFadd{Kimberly Hughes,
John Kelly, Dmitri Petrov, Campbell Rolian, Christian Schl}{\DIFadd{\"o}}\DIFadd{tterer}\DIFaddend . We would
also like to thank Doc Edge for helpful statistical advice, and \DIFdelbegin \DIFdel{Matt
Osmond}\DIFdelend \DIFaddbegin \DIFadd{Dave Begun}\DIFaddend ,
Erin Calfee, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{Sarah Friedman, Andy Kern, Chuck Langley, and Michael Turelli,
Matt Osmond, Peter Ralph, }\DIFaddend Sivan Yair for helpful discussions. \DIFaddbegin \DIFadd{Additionally, we
thank Guy Sella and an anonymous reviewer whose comments greatly improved the
manuscript. }\DIFaddend This research was supported by an NSF Graduate Research Fellowship
...
%DIFDELCMD < %%%
\DIFdelend \section*{Supplementary Material}
\DIFdelbegin %DIFDELCMD < \beginsupplement
%DIFDELCMD < %%%
...
\DIFaddbegin \section{\DIFadd{Estimator Bias Correction}}
\DIFaddend \subsection{\DIFdelbegin \DIFdel{Estimator Bias Correction}\DIFdelend \DIFaddbegin \DIFadd{Correcting variance bias with a single depth sampling process}\DIFaddend }
\DIFdelbegin \subsubsection{\DIFdel{Correcting variance bias with a single depth sampling process}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \label{supp:depth-var-corr}
...
\DIFdelbegin \subsubsection{\DIFdel{Correcting variance bias with individual and depth sampling processes}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Correcting variance bias with individual and depth sampling processes}}
\DIFaddend \label{supp:ind-depth-var-corr}
...
\DIFdelbegin \subsubsection{\DIFdel{Covariance Correction}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Covariance Correction}}
\DIFaddend \label{supp:cov-corr}
...
\DIFdelbegin \subsubsection{\DIFdel{Temporal-Replicate Covariance Matrix Correction}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Temporal-Replicate Covariance Matrix Correction}}
\DIFaddend \label{supp:matrix-correction}
...
\DIFdelbegin \subsubsection{\DIFdel{\textcite{Barghi2019-qy} Temporal Covariances}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{\textcite{Barghi2019-qy} Temporal Covariances}}
\DIFaddend \label{supp:barghi-covs}
...
diagonal element has the form \DIFdelbegin \DIFdel{$\var(\Delta p_{t_0}) = \sum_{i=0}^{t_0} \var(\Delta
p_{i}) + \sum_{i \ne j}^{t_0} \cov(\Delta p_{i}, \Delta p_{j})$}\DIFdelend \DIFaddbegin \DIFadd{$\var(\Delta p_{t_0}) = \sum_{i=0}^{t_0-1}
\var(\Delta p_{i}) + \sum_{i = 0}^{t_0 - 1} \sum_{j \ne i}^{t_0-1} \cov(\Delta
p_{i}, \Delta p_{j})$}\DIFaddend , and is thus a combination of the effects of drift and
...
\DIFdelbegin \subsection{\DIFdel{Block Bootstrap Procedure}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Block Bootstrap Procedure}}
\DIFaddend \label{supp:block-bootstrap}
...
which is the ratio of \DIFaddbegin \DIFadd{cumulative }\DIFaddend covariance to total variance, and the
...
\DIFdelbegin \subsection{\DIFdel{Replicate $G$ and Partitioning the Variance in Allele Frequency}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Replicate $G(t)$ and Partitioning the Variance in Allele Frequency}}
\DIFaddend \label{supp:replicate-g}

We define a statistic similar to \DIFdelbegin \DIFdel{$G$ }\DIFdelend \DIFaddbegin \DIFadd{$G(t)$ }\DIFaddend for estimating the proportion of allele
...
\DIFdelbegin %DIFDELCMD < \vb{In artificial selection studies with a control (non-selected) line, such as the
%DIFDELCMD < \textcite{Castro2019-uk} study, this allows us to estimate the contribution of
%DIFDELCMD < the effects of shared and unique indirect selection. In the case of this study,
%DIFDELCMD < we can estimate the drift, unique selection effect, and shared selection effect
%DIFDELCMD < terms using the fact that,
%DIFDELCMD < 

%DIFDELCMD < \begin{align*}
%DIFDELCMD <   \Delta p_{t,\mathrm{LS1}} &= \Delta_{_D} p_{t,\mathrm{LS1}} + \Delta_{_{U}} p_{t,\mathrm{LS1}} + \Delta_{_\mathrm{LS}} p_t \\ 
%DIFDELCMD <   \Delta p_{t,\mathrm{LS2}} &= \Delta_{_D} p_{t,\mathrm{LS2}} + \Delta_{_{U}} p_{t,\mathrm{LS2}} + \Delta_{_\mathrm{LS}} p_t \\ 
%DIFDELCMD <   \Delta p_{t,\mathrm{Ctrl}} &= \Delta_{_D} p_{t,\mathrm{Ctrl}}.
%DIFDELCMD < \end{align*}
%DIFDELCMD < 

%DIFDELCMD < Note that since the control replicate does not undergo artificial selection, we
%DIFDELCMD < assume that its allele frequency changes are determined entirely by genetic
%DIFDELCMD < drift. With free mating individuals (such as in a cage population), this may
%DIFDELCMD < not be the case, and sequencing adjacent generations would allow one to
%DIFDELCMD < differentiate the effects of selection and drift.
%DIFDELCMD < 

%DIFDELCMD < We assume that we can approximate the contribution of genetic drift in the
%DIFDELCMD < Longshanks selection lines with the observed variance in the control line, or
%DIFDELCMD < $\var(\Delta p_{t,\mathrm{Ctrl}}) = \var(\Delta_{_{D} p_{t,\mathrm{LS1}}}) =
%DIFDELCMD < \var(\Delta_{_{D} p_{t,\mathrm{LS2}}})$. Then, the combined effects of
%DIFDELCMD < selection can be estimated by averaging the variances of the two Longshanks
%DIFDELCMD < selection lines, and subtracting the variance in allele frequency change in the
%DIFDELCMD < control line, which we treat as driven by drift alone (since matings are
%DIFDELCMD < random). Note that each variance is bias-corrected according to the methods
%DIFDELCMD < described in Supplementary Materials \ref{supp:matrix-correction}, and the
%DIFDELCMD < average sequencing depths between lines are nearly identical. Thus, we have
%DIFDELCMD < 

%DIFDELCMD < \begin{align*}
%DIFDELCMD <   (\var(\Delta p_{t,\mathrm{LS1}}) + \var(\Delta p_{t,\mathrm{LS2}}))/2 - \var(\Delta p_{t,\mathrm{Ctrl}}) = 
%DIFDELCMD <   \overline{\var(\Delta_{_U} p_{t,\mathrm{LS}})} + \var(\Delta_{_\mathrm{LS}} p_t)
%DIFDELCMD < \end{align*}
%DIFDELCMD < %
%DIFDELCMD < where the bar indicates values averaged both Longshanks selection lines.
%DIFDELCMD < Additionally, use the fact that
%DIFDELCMD < 

%DIFDELCMD < \begin{align*}
%DIFDELCMD <   \cov(\Delta p_{t,\mathrm{LS1}}, \Delta p_{t,\mathrm{LS2}}) = \var(\Delta_{_\mathrm{LS}} p_t)
%DIFDELCMD < \end{align*}
%DIFDELCMD < %
%DIFDELCMD < we can also separate out the unique and shared components by subtracting off
%DIFDELCMD < this covariance,
%DIFDELCMD < 

%DIFDELCMD < \begin{align*}
%DIFDELCMD <   \overline{\var(\Delta_{_U} p_{t,\mathrm{LS}})}  = 
%DIFDELCMD < (\var(\Delta p_{t,\mathrm{LS1}}) + \var(\Delta p_{t,\mathrm{LS2}}))/2 - \var(\Delta p_{t,\mathrm{Ctrl}}) - \cov(\Delta p_{t,\mathrm{LS1}}, \Delta p_{t,\mathrm{LS2}}).
%DIFDELCMD < \end{align*}
%DIFDELCMD < 

%DIFDELCMD < Finally, we can divide each of these values by the total variance to get the
%DIFDELCMD < proportion of total variance drift, and unique and shared effects of selection
%DIFDELCMD < contribute towards the total. To derive confidence intervals for the estimates
%DIFDELCMD < of unique and shared effects of selection, we use a block bootstrap procedure
%DIFDELCMD < as described in Supplementary Materials Section \ref{supp:block-bootstrap}.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin {\DIFadd{In artificial selection studies with a control (non-selected) line, such as the
...
\DIFdelbegin \subsection{\DIFdel{The Empirical Neutral Null Windowed Covariance Distribution}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align}
  \DIFadd{\Delta p_{t,\mathrm{LS1}} }&\DIFadd{= \Delta_{_D} p_{t,\mathrm{LS1}} + \Delta_{_{U}} p_{t,\mathrm{LS1}} + \Delta_{_\mathrm{LS}} p_t }\\ 
  \DIFadd{\Delta p_{t,\mathrm{LS2}} }&\DIFadd{= \Delta_{_D} p_{t,\mathrm{LS2}} + \Delta_{_{U}} p_{t,\mathrm{LS2}} + \Delta_{_\mathrm{LS}} p_t }\\ 
  \DIFadd{\Delta p_{t,\mathrm{Ctrl}} }&\DIFadd{= \Delta_{_D} p_{t,\mathrm{Ctrl}}.
}\end{align}

\DIFadd{Note that since the control replicate does not undergo artificial selection, we
...
\DIFadd{We assume that we can approximate the contribution of genetic drift in the
...
\begin{align}
  \DIFadd{(\var(\Delta p_{t,\mathrm{LS1}}) + \var(\Delta p_{t,\mathrm{LS2}}))/2 - \var(\Delta p_{t,\mathrm{Ctrl}}) = 
  \overline{\var(\Delta_{_U} p_{t,\mathrm{LS}})} + \var(\Delta_{_\mathrm{LS}} p_t)
}\end{align}
%DIF > 
\DIFadd{where the bar indicates values averaged both Longshanks selection lines.
Additionally, use the fact that
}

\begin{align}
  \DIFadd{\cov(\Delta p_{t,\mathrm{LS1}}, \Delta p_{t,\mathrm{LS2}}) = \var(\Delta_{_\mathrm{LS}} p_t)
}\end{align}
%DIF > 
\DIFadd{we can also separate out the unique and shared components by subtracting off
this covariance,
}

\begin{align}
  \DIFadd{\overline{\var(\Delta_{_U} p_{t,\mathrm{LS}})}  = 
...
\DIFadd{Finally, we can divide each of these values by the total variance to get the
...
\section{\DIFadd{The Empirical Neutral Null Windowed Covariance Distribution}}
\DIFaddend \label{supp:empirical-null}
...
\DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{non-seasonal }\DIFaddend structure well, leading to an enrichment of small p-values.  \DIFdelbegin \DIFdel{This
...
}\DIFaddend non-seasonal \DIFaddbegin \DIFadd{structure may be due to a poorly calibrated null model.
Furthermore, non-seasonal }\DIFaddend temporal structure is also evident in our temporal
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \section{\DIFadd{Approximating the Reduction in Diversity from $G(t)$}}
\label{supp:reduction-approx}
\DIFaddend 

\DIFaddbegin \DIFadd{To help reconcile our measure of linked selection, $G(t)$, with familiar
...
= \var(p_t - p_0)$ be the total observed variation in allele frequency, $C_{LS}
= \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i, \Delta p_j)$ be the
contribution of all pairwise temporal covariances (observable), $V_D$ be the
unobservable drift-only variance in allele frequency, and $V_{LS} =
...
\begin{align}
  \DIFadd{V_{T} = V_{LS} + C_{LS} + V_D
}\end{align}
%DIF > 
\DIFadd{Our measure $G(t)$ is then,
}

\begin{align}
  \DIFadd{G(t) = \frac{C_{LS}}{V_T}
}\end{align}
%DIF > 
\DIFadd{meaning we can express the fraction of total variance due to drift alone as 
}

\begin{align}
  \DIFadd{\frac{V_D}{V_T} }&\DIFadd{= 1 - G(t) - \frac{V_{LS}}{V_T} }\\
                  &\DIFadd{= 1 - G(t) - \varepsilon }\\
                  & \DIFadd{\le 1 - G(t),
}\end{align}
%DIF > 
\DIFadd{where $\varepsilon \ge 0$ since these variances are positive. Throughout
this section, for convenience, we assume that the covariances contributing to
$C_{LS}$, and thus $G(t)$, are all positive.
}

\DIFadd{In a neutral Wright--Fisher population, the total variation in allele
frequency change over $t$ generations is
}

\begin{align}
  \DIFadd{V_T = \var(p_t - p_0) = p_0(1 - p_0) \left[ 1 - (1 - \nicefrac{1}{2N_e})^t \right].
}\end{align}
%DIF > 
\DIFadd{For small $t$, a common temporal estimator for the variance effective
population size $N_e$ is,
}

\begin{align}
  \DIFadd{N_e \approx \frac{t p_0(1-p_0)}{2 V_T}
}\end{align}

\DIFadd{Then, the drift-only $N_e$ estimate (that is, removing the effects of linked
selection) replaces the observable $V_T$ with unobservable $V_D$, and uses the
$G(t)$ estimate to bound this:
}

\begin{align}
  \DIFadd{N_{e,D} }&\DIFadd{\approx \frac{t p_0(1-p_0)}{2 V_T(1 - G(t) - \varepsilon)} }\\
          &\DIFadd{\gtrsim  \frac{t p_0(1-p_0)}{2 V_T(1 - G(t))} }\\
          &\DIFadd{\gtrsim  \frac{N_e}{1 - G(t)} }\\
  \DIFadd{\frac{N_e}{N_{e,D}} }&\DIFadd{\lesssim  1 - G(t)
}\end{align}

\DIFadd{In the linked selection literature, it is common to translate the impact of
...
\begin{align}
  \DIFadd{G(t) \lesssim 1 - \frac{N_e}{N_{e,D}} 
}\end{align}
\DIFadd{which shows that our measure $G(t)$ is a lower bound, over much shorter
timescales, to the familiar measure $1 -
\nicefrac{\bar{\pi}}{\pi_{0}}$. 
}

\DIFadd{\textcite{Elyashiv2016-vt} estimated that linked selection in }\emph{\DIFadd{Drosophila
  melanogaster}} \DIFadd{had resulted in a $1 - \nicefrac{\bar{\pi}}{\pi_{0}} = 77\%$
  reduction in the level of neutral diversity. Thus our estimate of $G(t)
  \approx 20\%$ in }\emph{\DIFadd{Drosophila simulans}} \DIFadd{is smaller than that seen over
long timespans in a closely related species. }}


%DIF >  Using another approximation, we can consider what roughly equivalently
%DIF >  reduction in diversity due to background selection is expected, using the
%DIF >  theory of Hudson and Kaplan (\citeyear{Hudson1994-oh},
%DIF >  \citeyear{Hudson1995-xc}). Their theory states, 

%DIF >  \begin{align}
%DIF >    \frac{N_e}{N_{e,D}} = \exp\left(-\frac{U}{R}\right)
%DIF >  \end{align}

%DIF >  where $U$ is the total genome-wide deleterious mutation rate per generation,
%DIF >  and $R$ is the total recombination map length in Morgans. For \emph{Drosophila
%DIF >  melanogaster} (we currently lack good estimates for \emph{D. simulans}), $R
%DIF >  \approx 3$ Morgans, implying a $G(t)$ of at least 20\% implies $U \gtrsim
%DIF >  0.67$. This is entirely consistent with recent work estimating $U \approx 1.6$
%DIF >  in \emph{D. melanogaster} from longer timescale population genetic data
%DIF >  \parencite{Elyashiv2016-vt} (their estimate of $U$ implies a $G(t) \approx
%DIF >  0.59$, or around 59\% reduction in neutral diversity




\section{\DIFadd{Simulation Results}}
\label{supp:forward}

\DIFadd{We conducted extensive simulations to understand how temporal covariance,
...
\DIFadd{Due to the high computational burden of forward simulations over this wide
...
analogous to a quarter of an autosome of }\emph{\DIFadd{Drosophila melanogaster}}\DIFadd{;
...
this length in }\emph{\DIFadd{D.  melanogaster}}\DIFadd{. Since our main goal is to understand
...
\DIFadd{All forward simulations were conducted using SLiM }\parencite{Haller2019-vu} \DIFadd{and
run and processed using Snakemake }\parencite{Koster2012-iv}\DIFadd{; all simulation
routines are available in the Github repository
}\url{https://github.com/vsbuffalo/cvtk/}\DIFadd{.
}

\subsection{\DIFadd{The Effects of the Genetic Architecture under Exponential Directional Selection}}
\label{supp:genetic-arch}

\DIFadd{We first investigated the effects of the selected trait's genetic architecture
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The temporal covariances $\cov(\Delta p_5, \Delta p_t)$ from the
...
\DIFadd{During this burnin, sites were either marked as neutral (with mutation rate
...
\DIFadd{Overall, we confirm a finding in \textcite{Buffalo2019-io} that the initial
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The $G(t)$ trajectories of 50 replicate simulations, across
...
\DIFadd{In our previous work, we did not investigate the affect of trait architecture
...
%DIF > \vb{TODO: note about different burnin pops?}


\subsection{\DIFadd{Temporal Covariances and $G(t)$ under Gaussian Stabilizing Selection}}

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\textwidth]{figures/gss-zbar.pdf}

  \caption{\DIFaddFL{The population mean trait value under the Gaussian stabilizing
...
\DIFadd{Additionally, we wanted to ensure that our temporal covariances and $G(t)$
...
the same two population simulation scheme described above (these shifts are
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{Mean temporal covariance ($\cov(\Delta p_5, \Delta p_t)$, with $t$ varying
...
\DIFadd{Overall, we see the same qualitative results under Gaussian stabilizing
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{$G(t)$ trajectories across 30 replicate Gaussian stabilizing
...
\DIFadd{Additionally, we looked at the effect the size of each replicate population
  ...
  replicate population sizes lead to }\emph{\DIFadd{larger}} \DIFadd{temporal covariances. We
...
temporal covariance as predicted by our theory }\parencite{Buffalo2019-io}\DIFadd{.
}
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{$G(t)$ trajectories under GSS after sudden optima shift of 1 at
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\textwidth]{figures/gss-covs-sampleN.pdf}

  \caption{\DIFaddFL{Temporal covariance $\cov(\Delta p_5, \Delta p_t)$, where $t$ varies
...
\subsection{\DIFadd{Convergence Correlations}}

\DIFadd{Using the same exponential fitness function simulations described above, we
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The convergence correlations across the two population line
...
\DIFadd{Overall, our simulations confirm our hypotheses; see Supplementary Material
...
\DIFadd{We also wanted to test whether we see similar convergence correlations under
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The convergence correlations across the two population line Gaussian
...
\subsection{\DIFadd{Sampling in Temporal Blocks}}
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{A: The $G(t)$ averaged over 50 replicate simulations with $V_A =
...
\DIFadd{In our analysis of the \textcite{Barghi2019-qy} data, we describe our statistic
...
cannot contribute to the numerator of $G(t)$ but contributes to the denominator,
...
variance terms }\parencite{Buffalo2019-io}\DIFadd{).  To verify that $G(t)$ estimated
...
\DIFadd{First, comparing $G(t)$ when sampling population frequencies every generation
...
frequency change between two generations (equation 26, \mbox{%DIFAUXCMD
\cite{Buffalo2019-io}}\hspace{0pt}%DIFAUXCMD
).
...
$t$'s, we see that $G(t)$ calculated every generation does not account for
linked selection's inflation of $\var(\Delta p_t)$ and underestimates the true
impact of linked selection as expected (dashed gray line in Supplementary
Figure \ref{suppfig:supp-blocks}A).
}

\DIFadd{To further understand the effects of calculating temporal covariances every ten
...
\subsection{\DIFadd{Background Selection}}

\DIFadd{In our previous work, \textcite{Buffalo2019-io}, we did not investigate whether
...
\DIFadd{We simulated background selection in a 50 megabase region, where deleterious
alleles are randomly introduced by mutation. Following background selection
literature
}\parencite{Charlesworth1993-gb,Nordborg1996-nq,Hudson1994-oh,Hudson1995-xc}\DIFadd{, we
...
are on the higher end of the spectrum expected for }\emph{\DIFadd{Drosophila}}\DIFadd{, but we
wanted to ensure the strength was sufficient to see a signal. Note that if $U
\approx 1.6$ }\parencite{Elyashiv2016-vt}\DIFadd{, and the }\emph{\DIFadd{Drosophila}} \DIFadd{genome is
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The trajectories of $G(t)$ through time under background selection,
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The temporal covariances, $\cov(\Delta p_{50}, \Delta p_t)$ (where
...
\DIFadd{Additionally, we investigated whether background selection can create
...
\DIFadd{Finally, we found in these background selection results that including or
excluding sites fixed or lost through time can lead  to differences in the
estimated $G(t)$ trajectories and temporal covariance. We discuss this
extensively in a subsequent section, Supplementary Materials Section
\ref{supp:fixation}.
}
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The convergence correlation created by background selection through
...
\subsection{\DIFadd{Truncation Selection}} 

\DIFadd{We also explored how directional truncation selection generates temporal
...
truncation selection generated more sustained positive covariances and so had a
larger long-term impact on the variance in allele frequency change.  We again
noticed fixation or loss of sites has a strong effect on temporal
...
see both the effect of handling fixed/lost sites differently and the
faster rate of drift in Supplementary Materials Figure
\ref{suppfig:supp-trunc}A, where weaker truncation selection actually has
higher levels of $G(t)$. Looking just at temporal covariances, we find that
stronger truncation selection (e.g.  a smaller tail of individuals selected)
does lead to greater temporal covariances. Overall, these truncation selection
simulations demonstrate the value of considering both absolute measures of
selection's effect on allele frequency changes, i.e. temporal covariance, as
well as measures relative to drift, i.e. the $G(t)$ trajectories. While
selecting a smaller tail of individuals is associated with }\emph{\DIFadd{stronger}}
\DIFadd{selection, it also is leading to a faster rate of drift, which can distort
conclusions inferred from considering the $G(t)$ trajectories alone.
}
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{$G(t)$ trajectories (A) and temporal covariances (B) from
    ...
    temporal covariances calculated }\emph{\DIFaddFL{including}} \DIFaddFL{fixed sites, while the
...
\subsection{\DIFadd{The Effect of Fixations in the Empirical Datasets}}
\label{supp:fixation}

\DIFadd{In our simulation results, we noticed the temporal covariances and $G(t)$
...
calculation can lead to biases. However, with }\emph{\DIFadd{sample}} \DIFadd{allele frequencies,
...
\DIFadd{With empirical data calculated on sample allele frequencies, low frequency
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{The effect of excluding fixed/lost sites in the calculation of the
...
\begin{figure}[!ht]
  ...
  \caption{\DIFaddFL{A version of Figure \ref{fig:figure-3} (A) and (B) excluding fixed
...
\DIFadd{To ensure that our findings were robust to handling sites with a frequency of
...
\DIFaddend \subsection{PCA of \textcite{Barghi2019-qy} replicates}
...
\DIFdelbegin \subsubsection{\DIFdel{\textcite{Barghi2019-qy} Temporal Covariances Per Replicate}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{\textcite{Barghi2019-qy} Temporal Covariances Per Replicate}}
