...
frequencies as some parents \vb{contribute} more or less copies of specific
...
replicate populations, are being used to detect \vb{large-effect} selected loci
...
polygenic linked selection detectable from \vb{temporal genomic data} is the
temporal autocovariance (i.e. covariance between two timepoints) in allele
frequency changes \parencite{Buffalo2019-io}.  These covariances \vbout{can be
estimated from temporal genomic data and} are created when the loci that
underly heritable fitness variation perturb the frequencies of linked alleles;
in contrast, when genetic drift acts alone in a closed population, these
covariances are zero in expectation for neutral alleles.  Mathematically,
temporal covariances are useful because it is natural to decompose the total
variance in allele frequency change across a \vb{time interval} into the
variances and covariances in allele frequency change \vb{between generations}.
Furthermore, biologically, these covariances reflect the extent to which allele
...
change (in ten-generation intervals, denoted $\Delta_{_{10}} p_t$) \vb{of} some
...
biases created due to sampling noise, and \vb{normalized} the entries for
...
decays (\vb{which could occur as a population reaches a new optimum})
...
we cannot \vb{observe covariances between pairs of generations that constitute
the estimated covariances between ten-generation time intervals; had these
covariances been measurable on shorter timescales, their cumulative effect may
have been higher yet (see Supplementary Material Sections
\ref{supp:barghi-covs} and \ref{supp:tempblocks} for more details).}

Since our covariances are essentially averages over loci, the covariance
estimate could be strongly affected by a few outlier regions. To test whether
large outlier regions drive the genome-wide signal we see in the
...
take the median windowed covariance, and trimmed-mean windowed covariance, as a
measure of the genome-wide covariance robust to large-effect loci. These robust
estimates (Supplementary Table \ref{supp:table-trimmed-mean} and Supplementary
Figure \ref{suppfig:barghi-trimmed-mean}) confirm the patterns we see using the
mean covariance, confirming that genomic temporal covariances are non-zero due
to the impact of selection acting across many genomic \vb{regions}. 
...
variance and covariance components, \vb{$\var(p_t - p_0) = \sum_{i=0}^{t-1}
  \var(\Delta p_i) + \sum_{i=0}^{t-1} \sum_{j \ne i}^{t-1} \cov(\Delta p_i,
\Delta p_j)$} (we \vb{bias-correct} these for sequencing depth), and the
...
sequenced every ten generations, \vb{the numerator uses the covariances
  estimated between ten-generation blocks of allele frequency change}; \vb{thus
  the strong, unobservable, covariances between adjacent generations do not
contribute to the numerator of $G(t)$}. Additionally, selection also inflates
...
$G(t)$ is \vb{considerably} conservative (\vb{we demonstrate this through
...
%\vb{This implies that the $N_e$
%without the effects of selection is at XXX $\nicefrac{}{(1-G(t))}$}
...
\textcite{Barghi2019-qy} data, we use $\Delta_{_{10}} p_t$. \vb{Between
...
positive \vb{convergence correlations}.  \vb{We find all three convergent
...
\vb{Additionally,} we can calculate the proportion of the total variance in
allele frequency change from convergent selection pressure, \vb{analogous to
...
experiment offers a useful control to test our \vb{convergence correlations}:
we expect to see \vb{significant} positive \vb{convergence correlations} in the
comparison between the two Longshanks selection lines, but not \vb{between each
of the control line and Longshanks line pairs}. We find that this is the case
(gray confidence intervals in Figure \ref{fig:figure-2}C), with \vb{convergence
correlations between each of the Longshanks lines to the control not being
statistically different from zero}, while the convergence correlation between
the two Longshanks lines is \vb{strong (0.18)} and statistically significant
(CIs [0.07, 0.25]).
...
This \vb{might} reflect either a change in the environment or the genetic
background, due to epistatic relationships among alleles altered by frequency
changes (\vb{which can occur during an optima shift; \cite{Hayward2019-kt}}) or
recombination breaking up selective alleles. Such reversals in \vb{selection
...
an empirical \vb{neutral} null distribution by randomly flipping the sign of
...
\ref{fig:figure-3} A and B) that there are an empirical excess of windows with
positive covariances between close timepoints compared to the null distribution
(a heavier right tail), and that this then shifts to an excess of windows with
negative covariances between more distant timepoints (a heavier left tail)
...
\vbout{arises and} sweeps to fixation, over four decades of theoretical and
empirical research has \vb{bettered} our understanding of linked selection.
One under-used approach to understand the genome-wide effects of selection on
\vb{polygenic trait (e.g. on standing variation, stems} from an early
...
populations, and found that a \vb{substantial} proportion of the response is
...
be seen as a strong lower bound of the effects of selection, \vb{which we have
...
population parameters such as the additive genetic variance for fitness
...
stronger or more sustained \vb{than} in natural populations
...
closed populations\vb{;} in natural populations temporal covariance could also
...
(Supplementary Figure \ref{suppfig:bergland-correction}).  Through our
...
seasonal fluctuating selection. \gc{Some fraction of the permutations happen to fit non-seasonal structure
well, leading to an enrichment of small p-values.  We note that
genetic drift is not accounted for in the model used to estimate the
significance of seasonally fluctuations and so some of these issues
from non-seasonal structure may be due to a poorly calibrated null model.  Furthermore,  }
...
\vb{expressions as a reduction} in neutral diversity, as parameterized by
...
\end{align}
%
where $\varepsilon \ge 0$ since these variances are positive. \vb{Throughout
