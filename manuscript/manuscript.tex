...
function. By using an exponential fitness function (\gc{corresponding
  to multiplicate}
selection across sites), we ensure there is no build up of LD due to
selection and epistasis \parencite{Turelli1990-kd,Bulmer1980-zo}
\gc{IS THIS PHRASED CORRECTLY, IS IT only in infinite pop that this holds?}; this serves
...
\ref{suppfig:sim-expfit-Gs}). When a trait is
gc{reasonably} monogenic ($L \approx 1$), $G(t)$
trajectories vary considerably across replicate lines, as certain
lines may \gc{stochastically lose } %notcontain
the few \gc{copies
  of the} selected alleles (top row of Supplementary Figure
\ref{suppfig:sim-expfit-Gs}). However, with a polygenic trait, ($L \ge 100$),
the $G(t)$ trajectories across replicates are \gc{similar} as each
replicate %that affect the trait's value
contains an abundance of \gc{trait} alleles (bottom rows of
...
beneficial alleles \gc{? Meaning lots of loci?}. We should note that our simplified simulation routines are
...
during burnin, \gc{followed by one of two} two optima shift routines: (1) sudden optima shifts of
...
loci), and the optima shift began at five generations after a $10N$
generation \gc{burnin}.
Across our GSS simulations, we see the expected selection response
(Supplementary Material Figure \ref{suppfig:gss-zbar}).

\gc{do you say what the effect size
  distribution is? Is it +/- again? How big is effect?}
