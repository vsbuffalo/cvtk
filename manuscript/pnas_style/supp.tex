...
function \vb{(where fitness of a trait value $z$ is $w(z) \propto \exp(x)$)}.
...
Overall, we \vb{again see a} finding in \textcite{Buffalo2019-io}: that the
...
contribution to the per-generation variance in allele frequency change (this is
the difference between our $G(t)$ and $G'(t)$ estimators, the latter of which
\vb{is defined in \textcite{Buffalo2019-io} and} includes these variance terms.
...
population size in the absence of selection. This allows us to estimate
$G'(t)$, which is a measure of $G(t)$ that accounts for the linked selection's
inflation of the variance in allele frequency change between two generations
\vb{($G'(t) = 1 - \nicefrac{t\E(p_0(1-p_0))}{2 \hat{N}\var(p_t - p_0)}$  see
equation 26 of \cite{Buffalo2019-io}} for more detail).  Plugging in the drift
effective population size $N_e = 1000$ into the expression for $G'(t)$ and
using the $\var(p_t - p_0)$ calculated for different $t$'s, we see that $G(t)$
calculated every generation does not account for linked selection's inflation
of $\var(\Delta p_t)$ and underestimates the true impact of linked selection as
