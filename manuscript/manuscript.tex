...
\begin{abstract}
 ...
 timepoints, as alleles are indirectly selected \gc{due} to remaining
 associations with selected \gc{alleles}. Since genetic drift does not lead to
 ...
 selection. Here, we reanalyze \vb{three selection experiments} to quantify the
...
with the indirect effect coming from the action of selection on \gc{correlated} %\vb{linked}
...
However, we lack an understanding of both how linked selection acts over
\vb{short time intervals and its full impact on genome-wide allele frequency
changes.}
...
across \vb{numerous} loci. This can lead to subtle allele frequency shifts on
standing variation that are difficult to distinguish from background levels of
genetic drift and sampling variance. \vb{Increasingly}, genomic experimental
...
timepoints is the temporal autocovariance (\vb{i.e. covariance between two
timepoints}) in allele frequency changes \parencite{Buffalo2019-io}.  These
covariances \vb{can be estimated} from temporal genomic data and are created
...
extent to which allele frequency changes in one generation predict changes in
...
drift. \vb{We estimate a lower bound of the fraction of variance in allele
...
to see how this quantity grows through time. \gc{When the sum of the covariances
  is positive this} measure can intuitively
  be understood as the relative fraction of allele frequency change normally
  thought of as ``drift" that is actually due to selection.
  %SO much of this felt redundent  the denominator of
  %$G(t)$ is $\var(p_t - p_0)$, which under drift alone is proportional to
  %$\nicefrac{1}{2 N_e}$.
 %By measuring the contribution of temporal covariances
%caused by selection, $G(t)$ looks at the fraction selection contributes to what
%looks like a faster rate of drift, or lower $N_e$.
Additionally, $G(t)$ can be understood as a short-timescale estimate of the reduction in neutral diversity
due to linked selection (\gc{or equivalently the reduction in neutral effective
population size needed to account linked selection,} see Supplementary Materials Section
...
apart; \vb{thus the strong covariances between adjacent generations are not
  observable and do not contribute to the numerator of $G(t)$}.
  ...
  conservative} (\vb{we demonstrate this through simulations in Supplementary
Material Section \ref{supp:tempblocks}}).  Still, we find a remarkably strong
...
an empirical null distribution by randomly flipping the \gc{sign }of the allele
frequency changes \gc{in each} genomic window \gc{(i.e. a single random sign
flip is applied to all loci in a window)}. This destroys the systematic covariances
...
\ref{fig:figure-3} A and B) \gc{that there are an empirical excess of windows
  with positive
  covariances between close timepoints compared to the null
  distribution (a heavier right tail), and that this then shifts to an excess of windows
  with negative covariances between more distant
timepoints (a heavier left tail)}
%that windowed temporal covariances between close
t%imepoints are skewed positive (a heavy right tail), while between more distant
%timepoints these windowed temporal covariances tend to shift to become more
%negative (a heavy left tail).
...
\ref{supp:forward} describes these simulation routines in \gc{more} detail as
...
selection trait variance $V_s = 1$ and an optima set at zero.
\gc{We note that the small burnin population size means that these
  simulations should not be taken as reflecting any specific natural
  population and they are for illustrative purposes only.}
We targeted a
